/* ==========================================================================
   CodeUp码上 · bytecode-editor.js — 独立「byte code 风格」编辑器（全功能 IDE）
   - 基于 Ace（CDN，失败时降级 textarea）
   - 多文件标签 / 左侧文件树 / 底部输出控制台 / 命令面板
   - 深度复用 CL.Runner 八语言运行引擎 + CodeUp码上 视觉主题
   ========================================================================== */
(function (global) {
  'use strict';
  ...
  /* ----------------------------------------------------------- 常量 */
  var ACE_VER = '1.36.5';
  var ACE_BASE = 'https://cdn.jsdelivr.net/npm/ace-builds@' + ACE_VER + '/src-min-noconflict/';
  var PRETTIER_URL = 'https://cdn.jsdelivr.net/npm/prettier@2.8.8/standalone.js';
  var STORE_KEY = 'codeup.bce.v1';
  var ACE_MODE = {
    python: 'python', java: 'java', c: 'c_cpp', cpp: 'c_cpp',
    javascript: 'javascript', typescript: 'typescript', go: 'golang', rust: 'rust',
    html: 'html', css: 'css', json: 'json', markdown: 'markdown'
  };
  var EXT_LANG = {
    py: 'python', pyw: 'python', java: 'java', c: 'c', h: 'cpp', cc: 'cpp', cpp: 'cpp',
    js: 'javascript', ts: 'typescript', jsx: 'javascript', tsx: 'typescript',
    go: 'go', rs: 'rust', html: 'html', htm: 'html', css: 'css', json: 'json', md: 'markdown'
  };
  var PRETTIER_PARSER = {
    javascript: 'babel', typescript: 'typescript', css: 'css',
    json: 'json', html: 'html', markdown: 'markdown'
  };

  /* ----------------------------------------------------------- 加载器 */
  var acePromise = null, prettierPromise = null;

  function loadAce() {
    if (acePromise) return acePromise;
    if (global.ace) {
      global.ace.config.set('basePath', ACE_BASE);
      global.ace.config.set('workerPath', ACE_BASE);
      acePromise = Promise.resolve(global.ace);
      return acePromise;
    }
    acePromise = new Promise(function (resolve, reject) {
      var s = document.createElement('script');
      s.src = ACE_BASE + 'ace.js';
      var done = false;
      var timer = setTimeout(function () { if (done) return; done = true; reject(new Error('Ace 加载超时')); }, 20000);
      s.onload = function () {
        ...
        if (global.ace) {
          ...
          global.ace.config.set('workerPath', ACE_BASE);
          resolve(global.ace);
        } else reject(new Error('Ace 未正确初始化'));
      };
      s.onerror = function () { if (done) return; done = true; clearTimeout(timer); reject(new Error('Ace 加载失败')); };
  ...
  function loadPrettier() {
    ...
    if (global.prettier) { prettierPromise = Promise.resolve(global.prettier); return prettierPromise; }
    prettierPromise = new Promise(function (resolve, reject) {
      var s = document.createElement('script');
      s.src = PRETTIER_URL;
      var done = false;
      var timer = setTimeout(function () { if (done) return; done = true; reject(new Error('fmt timeout')); }, 15000);
      s.onload = function () { if (done) return; done = true; clearTimeout(timer); global.prettier ? resolve(global.prettier) : reject(new Error('no prettier')); };
      s.onerror = function () { if (done) return; done = true; clearTimeout(timer); reject(new Error('fmt fail')); };
      document.head.appendChild(s);
    });
  ...
  /* -----------------------------------------------------------
     Ace 模块加载守卫
     Ace 的 mode/theme 模块是「异步脚本加载」：createEditSession 切换语言、
     setTheme 切换主题时都会走 ace.config.loadModule。这些请求可能在
     editor.destroy() 之后才 onload 完成，回调里会访问已被销毁的 session
     的 getDocument().getLength() —— 触发 "Cannot read properties of null"。
     这里对 loadModule 做包装：一旦本实例已销毁，就吞掉后续的模块回调，
     避免死 session 被触碰。
  ----------------------------------------------------------- */
  var _origLoadModule = null;
  function installLoadModuleGuard(ace, isDead) {
    if (!_origLoadModule) _origLoadModule = ace.config.loadModule.bind(ace.config);
    ace.config.loadModule = function (name, cb) {
      if (typeof cb !== 'function') return _origLoadModule(name, cb);
      return _origLoadModule(name, function (mod) {
        if (isDead()) return;                       // 编辑器已销毁 → 丢弃本次回调
        try { return cb(mod); }                     // 兜底：即便意外为空也吞掉崩溃
        catch (e) { /* swallow null-session crash */ }
      });
    };
  }
  function uninstallLoadModuleGuard(ace) {
    if (_origLoadModule && ace && ace.config) ace.config.loadModule = _origLoadModule;
  }
  ...
  function langFromName(name) { return EXT_LANG[extOf(name)] || 'javascript'; }
  function modeOf(lang) { return 'ace/mode/' + (ACE_MODE[lang] || 'text'); }
  // Ace 主题随网页深浅色联动
  function aceTheme() {
    var t = document.documentElement.getAttribute('data-theme');
    return t === 'light' ? 'ace/theme/tomorrow' : 'ace/theme/tomorrow_night';
  }

  /* =========================================================== 自定义下拉菜单 */
  function createCustomSelect(options, onChange) {
    // 创建容器
    var container = el('div', 'bce-custom-select');

    // 创建触发按钮
    var trigger = el('button', 'bce-select-trigger');
    trigger.type = 'button';
    var langIcon = el('span', 'bce-select-lang-icon', '');
    var selectText = el('span', 'bce-select-text', '');
    var arrow = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    arrow.setAttribute('class', 'bce-select-arrow');
    ...
    trigger.appendChild(langIcon);
    trigger.appendChild(selectText);
    trigger.appendChild(arrow);

    // 创建下拉菜单
    var dropdown = el('div', 'bce-select-dropdown');
    var searchWrap = el('div', 'bce-select-search');
    var searchInput = el('input', null);
    searchInput.type = 'text';
    searchInput.placeholder = '搜索语言…';
    searchWrap.appendChild(searchInput);
    var list = el('div', 'bce-select-list');
    var footer = el('div', 'bce-select-footer');
    footer.innerHTML = '<span>按</span><kbd>↑↓</kbd><span>选择</span><kbd>Enter</kbd><span>确认</span><kbd>Esc</kbd><span>关闭</span>';
    ...
    function renderList() {
      ...
      filteredOptions.forEach(function (opt, index) {
        var item = el('div', 'bce-select-item' + (opt.id === currentValue ? ' selected' : ''));
        item.dataset.value = opt.id;
        item.dataset.index = index;
        var itemIcon = el('span', 'bce-select-item-icon', opt.abbr || opt.name.charAt(0).toUpperCase());
        itemIcon.style.background = opt.color || '#0ea5e9';
        var itemText = el('span', 'bce-select-item-text', opt.name);
        var check = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        check.setAttribute('class', 'bce-select-item-check');
    ...
    function updateSelection() {
      var items = list.querySelectorAll('.bce-select-item');
      items.forEach(function (item, index) {
        if (index === selectedIndex) {
          item.style.background = 'var(--bg-hover, rgba(14, 165, 233, 0.08))';
    ...
    function updateTrigger() {
      var opt = options.find(function (o) { return o.id === currentValue; });
      if (opt) {
        langIcon.textContent = opt.abbr || opt.name.charAt(0).toUpperCase();
        langIcon.style.background = opt.color || '#0ea5e9';
        selectText.textContent = opt.name;
      }
    }

    // 打开下拉菜单（移到body中用fixed定位，避免被父元素overflow:hidden裁剪）
    function open() {
      if (isOpen) return;
      isOpen = true;
      ...
      var dropdownWidth = Math.max(rect.width, 200);
      ...
      selectedIndex = filteredOptions.findIndex(function (o) { return o.id === currentValue; });
    ...
    function selectOption(value) {
      ...
      listeners.change.forEach(function (fn) {
        fn.call(container, { target: container, type: 'change' });
      });
    }
      ...
      switch (e.key) {
        ...
        case 'Enter':
          ...
          if (selectedIndex >= 0 && filteredOptions[selectedIndex]) {
            selectOption(filteredOptions[selectedIndex].id);
          }
          break;
        case 'Escape':
          e.preventDefault();
    ...
    searchInput.addEventListener('input', function () {
      ...
      filteredOptions = options.filter(function (opt) {
        return opt.name.toLowerCase().includes(query) ||
               ...
               (opt.abbr && opt.abbr.toLowerCase().includes(query));
      });
      selectedIndex = 0;
      renderList();
      updateSelection();
    ...
    // 添加addEventListener方法
    container.addEventListener = function (type, fn) {
      if (type === 'change') {
        listeners.change.push(fn);
      } else {
        HTMLElement.prototype.addEventListener.call(this, type, fn);
      }
    };
  ...
  function Mount(root, opts) {
    opts = opts || {};
    // 支持「从代码卡片打开编辑器继续编辑」：消费挂载前由 openWith 写入的待编辑代码
    if (CL.BytecodeEditor && CL.BytecodeEditor.pending) {
      opts._pending = CL.BytecodeEditor.pending;
      CL.BytecodeEditor.pending = null;
    }
    var host = el('div', 'bce');
    ...
    var files = loadFiles();
    var activeId = null;
    var sessions = {};        // ace session 按文件 id 缓存
    var fallbackVals = {};    // textarea 降级时按 id 缓存
    var editor = null, usingFallback = false;
    var consoleLines = [];
    var paletteOpen = false;
    var palSel = 0;              // 命令面板当前选中项索引（键盘导航用）
    var destroyed = false;                          // 销毁后禁止任何异步模块回调触碰内核

    /* ---------- 构建 DOM ---------- */
    var bar = el('div', 'bce-bar');
    var barLeft = el('div', 'bce-bar-left');
    // 自定义下拉菜单组件（替代原生select）
    var langSel = createCustomSelect(CL.Languages.list, function (langId) {
      // 模拟change事件
      var event = new Event('change');
      langSel.dispatchEvent(event);
    });
    var nameInput = el('input', 'bce-name'); nameInput.spellcheck = false; nameInput.placeholder = '文件名';
    barLeft.appendChild(langSel); barLeft.appendChild(nameInput);

    var barRight = el('div', 'bce-bar-right');
    var runBtn = el('button', 'bce-btn bce-btn-accent', '▶ 运行 <kbd>Ctrl↵</kbd>');
    var fmtBtn = el('button', 'bce-btn', '格式化');
    var palBtn = el('button', 'bce-btn', '命令面板 <kbd>Ctrl⇧P</kbd>');
    var sideBtn = el('button', 'bce-btn bce-icon', '☰'); sideBtn.title = '切换侧边栏';
    var conBtn = el('button', 'bce-btn bce-icon', '▤'); conBtn.title = '切换控制台';
    var fsBtn = el('button', 'bce-btn bce-icon bce-fs', '⛶'); fsBtn.title = '全屏';
    var aiBtn = el('button', 'bce-btn bce-btn-ai', 'AI 助手'); aiBtn.title = 'AI 助手（代码解释/优化/修复）';
    barRight.appendChild(runBtn); barRight.appendChild(fmtBtn); barRight.appendChild(palBtn); barRight.appendChild(aiBtn);
    barRight.appendChild(sideBtn); barRight.appendChild(conBtn); barRight.appendChild(fsBtn);
    bar.appendChild(barLeft); bar.appendChild(barRight);

    var body = el('div', 'bce-body');
    var side = el('div', 'bce-side');
    var sideHead = el('div', 'bce-side-head', '<span>资源管理器</span>');
    var newBtn = el('button', 'bce-side-new', '＋ 新建');
    sideHead.appendChild(newBtn);
    var fileList = el('div', 'bce-filelist');
    side.appendChild(sideHead); side.appendChild(fileList);

    var main = el('div', 'bce-main');
    var tabs = el('div', 'bce-tabs');
    var editorHost = el('div', 'bce-editor-host');
    var console_ = el('div', 'bce-console');
    var conHead = el('div', 'bce-con-head', '<span class="bce-con-title">输出</span>');
    var clearBtn = el('button', 'bce-con-clear', '清空');
    conHead.appendChild(clearBtn);
    var conBody = el('div', 'bce-con-body');
    console_.appendChild(conHead); console_.appendChild(conBody);
    main.appendChild(tabs); main.appendChild(editorHost); main.appendChild(console_);

    body.appendChild(side); body.appendChild(main);
    host.appendChild(bar); host.appendChild(body);

    // 命令面板
    var palette = el('div', 'bce-palette');
    var palInput = el('input', 'bce-pal-input'); palInput.placeholder = '输入命令…';
    var palList = el('div', 'bce-pal-list');
    palette.appendChild(palInput); palette.appendChild(palList);
    host.appendChild(palette);

    // 模态框（语言切换确认等）
    var modal = el('div', 'bce-modal');
    var modalOverlay = el('div', 'bce-modal-overlay');
    var modalBox = el('div', 'bce-modal-box');
    var modalTitle = el('div', 'bce-modal-title');
    var modalBody = el('div', 'bce-modal-body');
    var modalFooter = el('div', 'bce-modal-footer');
    modalBox.appendChild(modalTitle); modalBox.appendChild(modalBody); modalBox.appendChild(modalFooter);
    modal.appendChild(modalOverlay); modal.appendChild(modalBox);
    host.appendChild(modal);

    // AI 面板 - 右侧常驻布局
    var aiPanel = el('div', 'bce-ai-panel');
    var aiHead = el('div', 'bce-ai-head', '<span>🤖 AI 助手</span>');
    var aiClose = el('button', 'bce-ai-close', '✕'); aiClose.title = '关闭';
    aiHead.appendChild(aiClose);

    // 输出区 - 顶部
    var aiOutputWrap = el('div', 'bce-ai-output-wrap');
    var aiOutput = el('div', 'bce-ai-output');
    aiOutputWrap.appendChild(aiOutput);

    // 底部控制区
    var aiFooter = el('div', 'bce-ai-footer');

    // 功能Tab
    var aiTabs = el('div', 'bce-ai-tabs');
    var aiTabExplain = el('button', 'bce-ai-tab active', '解释');
    var aiTabOptimize = el('button', 'bce-ai-tab', '优化');
    var aiTabComment = el('button', 'bce-ai-tab', '注释');
    var aiTabFix = el('button', 'bce-ai-tab', '修复');
    var aiTabGenerate = el('button', 'bce-ai-tab', '生成');
    var aiTabReview = el('button', 'bce-ai-tab', '审查');
    var aiTabTest = el('button', 'bce-ai-tab', '测试');
    var aiTabRefactor = el('button', 'bce-ai-tab', '重构');
    aiTabs.appendChild(aiTabExplain); aiTabs.appendChild(aiTabOptimize);
    aiTabs.appendChild(aiTabComment); aiTabs.appendChild(aiTabFix);
    aiTabs.appendChild(aiTabGenerate); aiTabs.appendChild(aiTabReview);
    aiTabs.appendChild(aiTabTest); aiTabs.appendChild(aiTabRefactor);

    // 预设词
    var aiPresets = el('div', 'bce-ai-presets');

    // 输入框 + 生成按钮
    var aiInputWrap = el('div', 'bce-ai-input-wrap');
    var aiInput = el('textarea', 'bce-ai-input'); aiInput.placeholder = '和AI聊聊，或输入需求…';
    var aiRun = el('button', 'bce-btn bce-btn-accent bce-ai-run', '➤');
    aiRun.title = '发送 (Enter)';
    aiInputWrap.appendChild(aiInput); aiInputWrap.appendChild(aiRun);

    aiFooter.appendChild(aiTabs);
    aiFooter.appendChild(aiPresets);
    aiFooter.appendChild(aiInputWrap);

    aiPanel.appendChild(aiHead);
    aiPanel.appendChild(aiOutputWrap);
    aiPanel.appendChild(aiFooter);
    host.appendChild(aiPanel);

    // AI对话历史
    var aiChatHistory = [];
    var aiCurrentTab = 'explain';

    /* ---------- 持久化 ---------- */
    var loadedFromCard = false;
    function loadFiles() {
      // 从代码卡片「打开编辑器继续编辑」带过来的代码：以它作为首个文件
      if (opts._pending) {
        loadedFromCard = true;
        var pL = CL.Languages.get(opts._pending.lang) || CL.Languages.list[0];
        return [{ id: uid(), name: pL.file, lang: pL.id, value: opts._pending.code || '', dirty: false }];
      }
      try {
        var raw = localStorage.getItem(STORE_KEY);
        if (raw) { var a = JSON.parse(raw); if (Array.isArray(a) && a.length) return a; }
      } catch (e) {}
      var lang = opts.initialLang || 'python';
      var L = CL.Languages.get(lang) || CL.Languages.list[0];
      return [{ id: uid(), name: L.file, lang: L.id, value: L.hello || '', dirty: false }];
    }
    function saveFiles() {
      try { localStorage.setItem(STORE_KEY, JSON.stringify(files.map(function (f) {
        return { id: f.id, name: f.name, lang: f.lang, value: f.value, dirty: !!f.dirty };
      }))); } catch (e) {}
    }

    /* ---------- 编辑器内核 ---------- */
    /* 等待宿主元素真正进入文档流后再挂载编辑器内核，
       避免 Ace 在 detached 节点上初始化导致的尺寸 / 会话异常 */
    function whenReady(cb) {
      if (editorHost.isConnected) return cb();
      var n = 0;
      var id = setInterval(function () {
        if (editorHost.isConnected) { clearInterval(id); cb(); }
        else if (n++ > 120) { clearInterval(id); }   // 宿主已卸载，放弃挂载
      }, 50);
    }

    function buildEditor() {
      loadAce().then(function (ace) {
        installLoadModuleGuard(ace, function () { return destroyed; });  // 防御销毁后的异步 mode/theme 回调
        whenReady(function () {
          if (!editorHost.isConnected) return;       // 已被销毁
          ...
          // 安装代码联想：8 语言关键字 / 内置函数 / 常用 API / 代码片段
          if (CL.BceCompleters && CL.BceCompleters.install) {
            CL.BceCompleters.install(editor, ace, function () {
              var f = activeFile();
              return f ? f.lang : null;
            });
          }
          editor.commands.addCommand({ name: 'runBce', bindKey: { win: 'Ctrl-Enter', mac: 'Cmd-Enter' }, exec: function () { runActive(); } });
          editor.commands.addCommand({ name: 'palBce', bindKey: { win: 'Ctrl-Shift-P', mac: 'Cmd-Shift-P' }, exec: function () { togglePalette(); } });
          editor.on('changeSession', syncActiveFromEditor);
          usingFallback = false;
          applyActiveToEditor();
          // 从代码卡片载入时的联动动画
          if (loadedFromCard) {
            setTimeout(function () {
              host.classList.add('bce-editor-loaded-from-card');
              showLoadedFromCardBanner();
              setTimeout(function () { host.classList.remove('bce-editor-loaded-from-card'); }, 800);
            }, 100);
          }
          if (global.ResizeObserver) {
            var ro = new ResizeObserver(function () { try { editor.resize(); } catch (e) {} });
            ro.observe(editorHost);
            editor._ro = ro;
          }
          if (global.MutationObserver) {
            var mo = new MutationObserver(function () { try { editor.setTheme(aceTheme()); } catch (e) {} });
    ...
    function buildFallback() {
      whenReady(function () {
        ...
        var ta = el('textarea', 'bce-textarea');
        ...
        editor = {
          ...
          setMode: function () {},
          setTheme: function () {},
          getSession: function () { return { setUseWorker: function () {} }; }
        };
        ta.addEventListener('input', function () { syncActiveFromEditor(); });
        ta.addEventListener('keydown', function (e) {
          if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); runActive(); }
          if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === 'P' || e.key === 'p')) { e.preventDefault(); togglePalette(); }
        });
        applyActiveToEditor();
        if (CL.toast) CL.toast('Ace 加载失败，已降级为纯文本编辑器', 'warn');
      });
    }

    /* 从代码卡片载入时的欢迎提示 */
    function showLoadedFromCardBanner() {
      var f = activeFile();
      var langName = f ? (CL.Languages.get(f.lang) || {}).name || f.lang : '代码';
      var banner = el('div', 'bce-loaded-from-card-banner');
      banner.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 4h6v6"/><path d="M20 4 11 13"/><path d="M19 13v5a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h5"/></svg>' +
        '<span>已从教程载入 ' + langName + ' 代码，可继续编辑运行</span>';
      banner.style.cssText = 'position:absolute;top:12px;left:50%;transform:translateX(-50%);z-index:100;display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:10px;background:var(--bg-card,#fff);border:1px solid var(--border,#e2e8f0);box-shadow:0 4px 16px rgba(0,0,0,0.1);font-size:13px;color:var(--text-secondary,#64748b);pointer-events:none;';
    ...
    /* ---------- 渲染侧栏 / 标签 ---------- */
    function activeFile() { return files.filter(function (f) { return f.id === activeId; })[0] || files[0]; }
    function renderFiles() {
      fileList.innerHTML = '';
      files.forEach(function (f) {
        var row = el('div', 'bce-file' + (f.id === activeId ? ' active' : ''));
        row.innerHTML = '<span class="bce-file-ico" style="--lc:' + ((CL.Languages.get(f.lang) || {}).color || '#888') + '">' + ((CL.Languages.get(f.lang) || {}).abbr || '·') + '</span>' +
          '<span class="bce-file-name">' + esc(f.name) + '</span>' + (f.dirty ? '<span class="bce-dot"></span>' : '');
        row.addEventListener('click', function () { setActive(f.id); });
        var del = el('span', 'bce-file-del', '✕');
        del.title = '删除';
        del.addEventListener('click', function (e) { e.stopPropagation(); removeFile(f.id); });
        row.appendChild(del);
    ...
    function renderTabs() {
      tabs.innerHTML = '';
      files.forEach(function (f) {
        var t = el('div', 'bce-tab' + (f.id === activeId ? ' active' : ''));
        t.innerHTML = '<span>' + esc(f.name) + '</span>' + (f.dirty ? '<i class="bce-tab-dot"></i>' : '');
        t.addEventListener('click', function () { setActive(f.id); });
        var x = el('span', 'bce-tab-x', '✕');
        x.addEventListener('click', function (e) { e.stopPropagation(); removeFile(f.id); });
        t.appendChild(x);
        tabs.appendChild(t);
      });
    }
    ...
    function setActive(id) {
      if (usingFallback && editor) fallbackVals[activeId] = editor.getValue();
      activeId = id;
      var f = activeFile();
    ...
    function applyActiveToEditor() {
      ...
      if (usingFallback) { editor.setValue(fallbackVals[f.id] != null ? fallbackVals[f.id] : f.value); return; }
      if (!editor || !editor.setSession) return;
      if (!sessions[f.id]) {
        var ace = global.ace;
        sessions[f.id] = ace.createEditSession(f.value, modeOf(f.lang));
        sessions[f.id].setUseWorker(false);
      }
      editor.setSession(sessions[f.id]);
      // 主题已在 buildEditor 中设置（随网页主题联动），此处不再重复 setTheme 以免触发多余的异步主题加载
    }
    function syncActiveFromEditor() {
      var f = activeFile(); if (!f) return;
      var v = editor.getValue();
      if (v !== f.value) { f.value = v; f.dirty = true; renderTabs(); renderFiles(); saveFiles(); }
    }
    function newFile() {
      var lang = langSel.value || 'python';
      var L = CL.Languages.get(lang) || CL.Languages.list[0];
      var base = L.file, n = 1, name = base;
      while (files.some(function (f) { return f.name === name; })) { n++; name = base.replace(/(\.[^.]+)?$/, n + '$1'); }
      var f = { id: uid(), name: name, lang: lang, value: '', dirty: false };
      files.push(f); activeId = f.id;
      langSel.value = lang; nameInput.value = name;
      renderTabs(); renderFiles(); saveFiles(); applyActiveToEditor();
    ...
    function removeFile(id) {
      ...
      if (idx < 0) return;
      files.splice(idx, 1);
      delete sessions[id]; delete fallbackVals[id];
      if (!files.length) { var L = CL.Languages.list[0]; files.push({ id: uid(), name: L.file, lang: L.id, value: '', dirty: false }); }
      if (activeId === id) activeId = files[0].id;
      renderTabs(); renderFiles(); saveFiles(); applyActiveToEditor();
    }
    function saveActive() {
      var f = activeFile(); if (!f) return;
      // f.value 始终与编辑器同步（见 syncActiveFromEditor），此处直接落盘即可
      f.dirty = false; renderTabs(); renderFiles(); saveFiles();
      if (CL.toast) CL.toast('已保存 ' + f.name, 'ok');
    }

    /* ---------- 运行 ---------- */
    function clearConsole() { consoleLines = []; renderConsole(); }
    function renderConsole() {
      if (!consoleLines.length) { conBody.innerHTML = '<span class="cc-empty">点击「运行」查看输出</span>'; return; }
      conBody.innerHTML = consoleLines.map(function (l) {
        return '<span class="cc-line ' + (l.cls || '') + '">' + esc(l.text) + '</span>';
      }).join('');
    ...
    function runActive() {
      ...
      if (CL.Runner && CL.Runner.enabled && !CL.Runner.enabled()) {
        clearConsole();
        consoleLines.push({ text: '⏳ 在线运行功能敬请期待', cls: 'sys' });
        renderConsole();
        return;
      }

      if (!CL.Languages.exists(f.lang)) {
        clearConsole();
        consoleLines.push({ text: '该文件类型（' + f.lang + '）暂不支持在线运行。', cls: 'warn' });
        renderConsole();
        if (CL.toast) CL.toast('该文件类型暂不支持在线运行', 'warn');
        return;
      }
      var code = f.value;                          // 文件内容即唯一数据源，不依赖编辑器内核
      f.value = code; f.dirty = false; renderTabs(); renderFiles(); saveFiles();
      clearConsole();
      consoleLines.push({ text: '▶ 运行 ' + f.name + ' …', cls: 'sys' });

      // 语法检查（运行前静态分析）
      var syntaxResult = checkSyntax(code, f.lang);
      if (!syntaxResult.ok) {
        consoleLines.push({ text: '⚠ 语法检查发现 ' + syntaxResult.errors.length + ' 个错误、' + syntaxResult.warnings.length + ' 个警告：', cls: 'warn' });
        syntaxResult.errors.forEach(function(e) {
          consoleLines.push({ text: '  ✗ 第' + e.line + '行' + (e.col ? '第' + e.col + '列' : '') + '：' + e.msg, cls: 'err' });
        });
        syntaxResult.warnings.forEach(function(w) {
          consoleLines.push({ text: '  ⚠ 第' + w.line + '行：' + w.msg, cls: 'warn' });
        });
        consoleLines.push({ text: '（仍将尝试运行，部分错误可能导致运行失败）', cls: 'sys' });
      }

      renderConsole();
      runBtn.disabled = true;
      var t0 = performance.now();
      CL.Runner.run(f.lang, code, {
        onProgress: function (m) { consoleLines.push({ text: m, cls: 'sys' }); renderConsole(); }
      }).then(function (res) {
        (res.lines || []).forEach(function (l) { consoleLines.push({ text: l.text, cls: l.cls || '' }); });
        consoleLines.push({ text: '—— 耗时 ' + (res.time != null ? res.time : Math.round(performance.now() - t0)) + 'ms' +
          (res.compiler ? ' · ' + res.compiler : '') + ' ——', cls: 'sys' });
        renderConsole();
        runBtn.disabled = false;
      }).catch(function (e) {
        consoleLines.push({ text: '运行异常：' + (e && e.message || e), cls: 'err' });
        renderConsole(); runBtn.disabled = false;
      });
    }
    ...
    function formatActive() {
      var f = activeFile(); if (!f) return;
      var code = f.value;                           // 数据源为文件内容
      var parser = PRETTIER_PARSER[f.lang];
      if (!parser) { if (CL.toast) CL.toast('该语言暂不支持在线格式化（仅做空白整理）', 'warn'); return doTidy(code, f); }
      fmtBtn.disabled = true;
      loadPrettier().then(function (prettier) {
        try {
          var out = prettier.format(code, { parser: parser, semi: true, singleQuote: false, printWidth: 100 });
          setCode(out); if (CL.toast) CL.toast('已用 Prettier 格式化', 'ok');
        } catch (e2) { if (CL.toast) CL.toast('格式化失败：' + e2.message, 'err'); }
        fmtBtn.disabled = false;
      }).catch(function () { if (CL.toast) CL.toast('Prettier 加载失败，仅做空白整理', 'warn'); doTidy(code, f); fmtBtn.disabled = false; });
    }
    function doTidy(code, f) {
      var out = code.replace(/[ \t]+$/gm, '').replace(/\n{3,}/g, '\n\n');
      if (out && !/\n$/.test(out)) out += '\n';
      setCode(out);
    }
    function setCode(v) {
      var f = activeFile(); if (!f) return;
      f.value = v;                                  // 数据源先行，内核就绪后再同步视图
      if (usingFallback) editor.setValue(v);
      else if (sessions[activeId]) sessions[activeId].setValue(v);
      else if (editor && editor.setValue) editor.setValue(v);
      syncActiveFromEditor();
    }
    function getCode() {
      var f = activeFile(); if (!f) return '';
      if (usingFallback && editor) return editor.getValue();
      if (sessions[activeId]) return sessions[activeId].getValue();
    ...
    /* ---------- 命令面板 ---------- */
    var COMMANDS = [
      { t: '运行代码', run: runActive },
      { t: '保存文件', run: saveActive },
      { t: '新建文件', run: newFile },
      { t: '格式化代码', run: formatActive },
      { t: '清空输出', run: clearConsole }
    ];
    CL.Languages.list.forEach(function (l) {
      COMMANDS.push({ t: '切换语言：' + l.name, run: function () { langSel.value = l.id; changeLang(); } });
    });
    COMMANDS.push({ t: '切换浅色/深色主题', run: function () { var btn = document.getElementById('themeToggle'); if (btn) btn.click(); } });

    function renderPalette(filter) {
      var q = (filter || '').toLowerCase();
      var list = COMMANDS.filter(function (c) { return c.t.toLowerCase().indexOf(q) >= 0; });
      if (palSel < 0) palSel = 0;
      if (palSel > list.length - 1) palSel = Math.max(0, list.length - 1);
      palList.innerHTML = '';
      list.slice(0, 12).forEach(function (c, i) {
        var row = el('div', 'bce-pal-item' + (i === palSel ? ' sel' : ''), esc(c.t));
        row.addEventListener('click', function () { closePalette(); c.run(); });
        row.addEventListener('mousemove', function () {
          if (palSel === i) return;
          var prev = palList.querySelector('.bce-pal-item.sel');
          if (prev) prev.classList.remove('sel');
          palSel = i; row.classList.add('sel');
        });
        palList.appendChild(row);
      });
      if (!list.length) palList.innerHTML = '<div class="bce-pal-empty">无匹配命令</div>';
    }
    function openPalette() { paletteOpen = true; palette.classList.add('open'); palInput.value = ''; palSel = 0; renderPalette(''); palInput.focus(); }
    ...
    function showModal(title, bodyHtml, confirmText, onConfirm, cancelText) {
      ...
      var cancelBtn = el('button', 'bce-btn', cancelText || '取消');
      var confirmBtn = el('button', 'bce-btn bce-btn-accent', confirmText || '确认');
      cancelBtn.addEventListener('click', hideModal);
      confirmBtn.addEventListener('click', function () { hideModal(); if (onConfirm) onConfirm(); });
      modalFooter.appendChild(cancelBtn); modalFooter.appendChild(confirmBtn);
    ...
    function convertCode(code, from, to) {
      ...
      // 提取代码中的字符串字面量（避免被转换规则破坏）
      var strings = [];
      var protectedCode = code.replace(/(["'`])(?:\\.|[^\\])*?\1/g, function(m) {
        strings.push(m);
        return '__STR_' + (strings.length - 1) + '__';
      });

      // 转换注释
      protectedCode = convertComments(protectedCode, from, to);

      // 转换打印输出
      protectedCode = convertPrint(protectedCode, from, to);

      // 转换变量声明
      protectedCode = convertVariables(protectedCode, from, to);

      // 转换函数定义
      protectedCode = convertFunctions(protectedCode, from, to);

      // 转换返回语句
      protectedCode = convertReturn(protectedCode, from, to);

      // 转换条件语句
      protectedCode = convertConditionals(protectedCode, from, to);

      // 转换循环语句
      protectedCode = convertLoops(protectedCode, from, to);

      // 恢复字符串字面量
      var result = protectedCode.replace(/__STR_(\d+)__/g, function(m, i) {
        return strings[parseInt(i)] || m;
      });

      // 去除多余分号
      result = result.replace(/;{2,}/g, ';');

      // 如果转换后代码变化不大，返回目标语言模板
      if (result.trim() === code.trim() && templates[to]) {
        var langName = CL.Languages.get(to) ? CL.Languages.get(to).name : to;
        return '// 自动转换：以下为 ' + langName + ' 示例代码\n' + templates[to];
      }

      // 包装代码：对于需要入口函数的语言，自动添加include和main函数
      result = wrapCode(result, to);

      return result;
    }

    // 包装代码：为需要入口函数的语言自动添加完整模板
    function wrapCode(code, lang) {
      // 检查是否已经包含main函数或类定义
      var hasMain = /\b(main\s*\(|int\s+main\s*\(|func\s+main\s*\(|fn\s+main\s*\(|public\s+static\s+void\s+main)/.test(code);
      if (hasMain) return code;

      // 检查是否有函数定义（如果有，不包装，用户可能自己写了完整代码）
      var hasFunction = /\b(function|def|func|fn)\s+\w+\s*\(/.test(code) ||
                        /\b(public|private|protected)?\s*(static)?\s*\w+\s+\w+\s*\([^)]*\)\s*\{/.test(code);
      if (hasFunction && lang !== 'python' && lang !== 'javascript' && lang !== 'typescript') {
        // 有函数定义但没有main，可能需要添加main调用，但这里简单处理：不包装
        return code;
      }
      ...
      var body = codeLines.map(function(l) { return '    ' + l; }).join('\n');
      var comments = commentLines.join('\n');
    ...
    // 转换注释
    function convertComments(code, from, to) {
      if (to === 'python') {
        // 其他语言转Python：// 注释 -> # 注释
        code = code.replace(/^\s*\/\/(.*)$/gm, function(m, c) { return '#' + c; });
        // /* */ 块注释 -> # 注释
    ...
    // 转换打印输出
    function convertPrint(code, from, to) {
      var printPatterns = {
        python: /print\s*\(([^)]*)\)/g,
        ...
        c: /printf\s*\(([^)]*)\)/g,
        cpp: /std::cout\s*<<\s*([^;]*?)(?:\s*<<\s*std::endl)?\s*;/g,
        go: /fmt\.Println\s*\(([^)]*)\)/g,
        rust: /println!\s*\(([^)]*)\)/g
      };

      var printReplace = {
        python: function(arg) { return 'print(' + arg + ')'; },
        javascript: function(arg) { return 'console.log(' + arg + ');'; },
        typescript: function(arg) { return 'console.log(' + arg + ');'; },
        java: function(arg) { return 'System.out.println(' + arg + ');'; },
        c: function(arg) {
          // 处理字符串字面量，添加\n
          if (/^__STR_\d+__$/.test(arg.trim())) {
            return 'printf(' + arg.trim().replace(/__STR_(\d+)__/g, function(m, i) { return '__STR_' + i + '__'; }) + ');';
          }
          return 'printf(' + arg + ');';
        },
        cpp: function(arg) { return 'std::cout << ' + arg + ' << std::endl;'; },
        go: function(arg) { return 'fmt.Println(' + arg + ')'; },
        rust: function(arg) { return 'println!(' + arg + ');'; }
      };

      var pattern = printPatterns[from];
      if (pattern && printReplace[to]) {
        code = code.replace(pattern, function(m, arg) {
          return printReplace[to](arg.trim());
        });
      }
      return code;
    }
    ...
    // 转换变量声明
    function convertVariables(code, from, to) {
      // JavaScript/TypeScript: let/var/const x = value;
      if (from === 'javascript' || from === 'typescript') {
        code = code.replace(/\b(?:let|var|const)\s+(\w+)\s*=\s*([^;]+);/g, function(m, name, value) {
          return varToLang(name, value, to);
        });
      }
      // Python: x = value
      else if (from === 'python') {
        code = code.replace(/^\s*(\w+)\s*=\s*(.+)$/gm, function(m, name, value) {
          if (/^(def|class|if|for|while|import|from|return|print|else|elif|try|except|finally|with|as|pass|break|continue|raise|yield|global|nonlocal|lambda|assert|del|in|is|not|and|or|True|False|None)$/i.test(name)) return m;
          return varToLang(name, value.trim(), to);
        });
      }
      // Java/C/C++: type x = value;
      else if (from === 'java' || from === 'c' || from === 'cpp') {
        code = code.replace(/\b(int|float|double|char|string|String|bool|boolean|long|short|byte|void)\s+(\w+)\s*=\s*([^;]+);/g, function(m, type, name, value) {
          return varToLang(name, value.trim(), to);
        });
      }
      // Go: x := value / var x type = value
      else if (from === 'go') {
        code = code.replace(/(\w+)\s*:=\s*([^;]+);?/g, function(m, name, value) {
          return varToLang(name, value.trim(), to);
        });
      }
      // Rust: let x = value; / let mut x = value;
      else if (from === 'rust') {
        code = code.replace(/\blet\s+(?:mut\s+)?(\w+)\s*=\s*([^;]+);/g, function(m, name, value) {
          return varToLang(name, value.trim(), to);
        });
      }
      return code;
    ...
    function varToLang(name, value, to) {
      switch (to) {
        case 'python': return name + ' = ' + value;
        case 'javascript': return 'let ' + name + ' = ' + value + ';';
        case 'typescript': return 'let ' + name + ' = ' + value + ';';
        case 'java': return 'var ' + name + ' = ' + value + ';';
        case 'c': return 'auto ' + name + ' = ' + value + ';';
        case 'cpp': return 'auto ' + name + ' = ' + value + ';';
        case 'go': return name + ' := ' + value;
        case 'rust': return 'let ' + name + ' = ' + value + ';';
        default: return name + ' = ' + value;
      }
    }

    // 转换函数定义
    function convertFunctions(code, from, to) {
      // JavaScript: function name(params) {}
      if (from === 'javascript' || from === 'typescript') {
        code = code.replace(/function\s+(\w+)\s*\(([^)]*)\)\s*(?::\s*\w+)?\s*\{/g, function(m, name, params) {
          return funcToLang(name, params, to);
        });
      }
      // Python: def name(params):
      else if (from === 'python') {
        code = code.replace(/def\s+(\w+)\s*\(([^)]*)\)\s*:/g, function(m, name, params) {
          return funcToLang(name, params, to);
        });
      }
      // Java: retType name(params) {}
      else if (from === 'java') {
        code = code.replace(/(?:public|private|protected)?\s*(?:static\s+)?(\w+)\s+(\w+)\s*\(([^)]*)\)\s*\{/g, function(m, retType, name, params) {
          if (name === 'main') return m;
          return funcToLang(name, params, to);
        });
      }
      // C/C++: retType name(params) {}
      else if (from === 'c' || from === 'cpp') {
        code = code.replace(/(\w+)\s+(\w+)\s*\(([^)]*)\)\s*\{/g, function(m, retType, name, params) {
          if (name === 'main') return m;
          return funcToLang(name, params, to);
        });
      }
      // Go: func name(params) retType {}
      else if (from === 'go') {
        code = code.replace(/func\s+(\w+)\s*\(([^)]*)\)\s*(?:\w+)?\s*\{/g, function(m, name, params) {
          if (name === 'main') return m;
          return funcToLang(name, params, to);
        });
      }
      // Rust: fn name(params) -> retType {}
      else if (from === 'rust') {
        code = code.replace(/fn\s+(\w+)\s*\(([^)]*)\)\s*(?:->\s*\w+)?\s*\{/g, function(m, name, params) {
          if (name === 'main') return m;
          return funcToLang(name, params, to);
        });
      }
    ...
    function funcToLang(name, params, to) {
      switch (to) {
        case 'python': return 'def ' + name + '(' + params + '):';
        case 'javascript': return 'function ' + name + '(' + params + ') {';
        case 'typescript': return 'function ' + name + '(' + params + ') {';
        case 'java': return 'void ' + name + '(' + params + ') {';
        case 'c': return 'void ' + name + '(' + params + ') {';
        case 'cpp': return 'void ' + name + '(' + params + ') {';
    ...
    // 转换返回语句
    function convertReturn(code, from, to) {
      if (to === 'python') {
        code = code.replace(/return\s+([^;]+);/g, 'return $1');
      } else if (from === 'python') {
        code = code.replace(/^\s*return\s+(.+)$/gm, 'return $1;');
      }
      return code;
    }

    // 转换条件语句
    function convertConditionals(code, from, to) {
      if (to === 'python') {
    ...
    // 转换循环语句
    function convertLoops(code, from, to) {
      if (to === 'python') {
        // while (condition) { -> while condition:
        code = code.replace(/while\s*\(([^)]+)\)\s*\{/g, 'while $1:');
        // for (let i = 0; i < n; i++) { -> for i in range(n):
        code = code.replace(/for\s*\(\s*(?:let|var|int)\s+(\w+)\s*=\s*(\d+)\s*;\s*\1\s*<\s*(\w+)\s*;\s*\1\+\+\s*\)\s*\{/g, 'for $1 in range($3):');
      } else if (from === 'python') {
        code = code.replace(/while\s+(.+):/g, 'while ($1) {');
        code = code.replace(/for\s+(\w+)\s+in\s+range\((\w+)\):/g, 'for (let $1 = 0; $1 < $2; $1++) {');
      }
      return code;
    }
    ...
    function changeLang() {
      ...
      var oldCode = getCode();
      // 先恢复下拉框为旧值，等用户确认后再真正切换
      langSel.value = oldLang;
      // 显示确认弹窗
      showModal(
        '切换语言',
        '<p>是否将当前代码转换为 <strong>' + esc(CL.Languages.get(newLang).name) + '</strong> 的等效代码？</p>' +
        '<p style="margin-top:8px;color:var(--fg-mute);font-size:13px">选择「转换代码」将自动替换为目标语言的等效代码；选择「仅切换语言」将保留原代码。</p>',
        '转换代码',
        function () {
          doChangeLang(f, newLang, oldCode, true);
        },
        '仅切换语言'
      );
      // 取消按钮：仅切换语言，保留原代码
      var cancelBtn = modalFooter.querySelector('.bce-btn:first-child');
      if (cancelBtn) {
        cancelBtn.addEventListener('click', function () {
          doChangeLang(f, newLang, oldCode, false);
        });
      }
    }
    function doChangeLang(f, newLang, oldCode, convert) {
      var oldLang = f.lang;
      f.lang = newLang;
      langSel.value = newLang;
      if (convert) {
        var converted = convertCode(oldCode, oldLang, newLang);
        setCode(converted);
        f.value = converted;
      }
      updateFileName(f);
      if (!usingFallback && sessions[f.id]) { delete sessions[f.id]; applyActiveToEditor(); }
      else applyActiveToEditor();
      renderTabs(); renderFiles(); saveFiles();
    }
    function updateFileName(f) {
      if (!f.name || /\.(py|java|c|cpp|cc|h|js|ts|jsx|tsx|go|rs|html|htm|css|json|md)$/i.test(f.name)) {
        var L = CL.Languages.get(f.lang);
        if (L) f.name = f.name.replace(/\.[^.]+$/, '') + '.' + L.ext;
      }
      nameInput.value = f.name;
    }
    ...
    nameInput.addEventListener('input', function () {
      var f = activeFile(); if (!f) return;
      f.name = nameInput.value; f.lang = langFromName(f.name);
      langSel.value = f.lang; renderTabs(); renderFiles(); saveFiles();
    });
    runBtn.addEventListener('click', runActive);
    fmtBtn.addEventListener('click', formatActive);
    palBtn.addEventListener('click', togglePalette);
    newBtn.addEventListener('click', newFile);
    clearBtn.addEventListener('click', clearConsole);
    sideBtn.addEventListener('click', function () { host.classList.toggle('no-side'); });
    conBtn.addEventListener('click', function () { host.classList.toggle('no-con'); });
    fsBtn.addEventListener('click', toggleFullscreen);
    aiBtn.addEventListener('click', toggleAIPanel);
    aiClose.addEventListener('click', function () { closeAIPanel(); });
    var allAITabs = [aiTabExplain, aiTabOptimize, aiTabComment, aiTabFix, aiTabGenerate, aiTabReview, aiTabTest, aiTabRefactor];

    // 预设词配置
    var aiPresetsMap = {
      explain: ['这段代码做什么', '解释核心逻辑', '复杂度分析', '逐行解释'],
      optimize: ['优化性能', '提升可读性', '减少内存占用', '简化代码'],
      comment: ['添加详细注释', '函数文档注释', '行内注释', '中文注释'],
      fix: ['修复语法错误', '修复潜在bug', '修复内存泄漏', '修复边界情况'],
      generate: ['快速排序', '斐波那契数列', '二分查找', '链表实现', '栈实现'],
      review: ['安全审查', '性能审查', '代码质量', '最佳实践检查'],
      test: ['生成单元测试', '边界测试', '异常测试', '性能测试'],
      refactor: ['提取函数', '重命名变量', '简化嵌套', '移除重复代码']
    };

    function renderPresets(tab) {
      var presets = aiPresetsMap[tab] || [];
      aiPresets.innerHTML = '';
      presets.forEach(function(p) {
        var btn = el('button', 'bce-ai-preset', p);
        btn.addEventListener('click', function() {
          aiInput.value = p;
          aiInput.focus();
        });
        aiPresets.appendChild(btn);
      });
      aiPresets.classList.add('show');
    }

    allAITabs.forEach(function (tab) {
      tab.addEventListener('click', function () {
        allAITabs.forEach(function (t) { t.classList.remove('active'); });
        tab.classList.add('active');
        var tabName = tab.textContent.replace(/\s/g, '');
        var tabMap = { '解释': 'explain', '优化': 'optimize', '注释': 'comment', '修复': 'fix', '生成': 'generate', '审查': 'review', '测试': 'test', '重构': 'refactor' };
        aiCurrentTab = tabMap[tabName] || tabName;
        aiOutput.innerHTML = '';
        renderPresets(aiCurrentTab);
        var placeholders = {
          explain: '问AI关于这段代码的问题…',
          optimize: '描述你希望优化的方向…',
          comment: '选择注释风格，或直接描述需求…',
          fix: '描述你遇到的问题或错误…',
          generate: '描述你想要生成的代码功能…',
          review: '选择审查重点，或直接描述需求…',
          test: '选择测试类型，或描述测试需求…',
          refactor: '选择重构类型，或描述重构目标…'
        };
        aiInput.placeholder = placeholders[aiCurrentTab] || '和AI聊聊…';
      });
    });

    // 初始化预设词
    renderPresets('explain');
    ...
    function runAI() {
      var code = getCode();
      var f = activeFile();
      var lang = f ? f.lang : 'python';
      var userQ = aiInput.value.trim();

      if (!userQ && aiCurrentTab === 'explain') {
        // 解释代码tab且无输入时，直接解释当前代码
        userQ = '解释这段代码';
      }

      // 添加用户消息到对话历史
      var userMsg = { role: 'user', content: userQ, time: new Date().toLocaleTimeString('zh-CN', {hour: '2-digit', minute: '2-digit'}) };
      ...
      typingEl.className = 'bce-ai-msg bce-ai-msg-ai';
      typingEl.innerHTML = '<div class="bce-ai-msg-bubble"><span class="bce-ai-spin"></span> 思考中…</div>';
      aiOutput.appendChild(typingEl);
      aiOutputWrap.scrollTop = aiOutputWrap.scrollHeight;
      ...
      setTimeout(function () {
        ...
        // 功能tab下优先调用功能函数，避免aiChat匹配到功能关键词返回预设回答
        var functionalTabs = ['explain', 'optimize', 'comment', 'fix', 'generate', 'review', 'test', 'refactor'];
        var isFunctionalTab = functionalTabs.indexOf(aiCurrentTab) !== -1;
        var functionalKeywords = /解释|优化|注释|修复|生成|审查|测试|重构|这段代码/;
        var chatResult = (!isFunctionalTab || !functionalKeywords.test(userQ)) ? aiChat(userQ, code, lang) : null;
        var result = '';
        ...
        if (chatResult) {
          ...
          // 功能模式
          if (aiCurrentTab === 'explain') {
            result = aiExplain(code, lang, userQ);
          } else if (aiCurrentTab === 'optimize') {
            result = aiOptimize(code, lang);
          } else if (aiCurrentTab === 'comment') {
            result = aiComment(code, lang);
          } else if (aiCurrentTab === 'fix') {
            result = aiFix(code, lang);
          } else if (aiCurrentTab === 'generate') {
            result = aiGenerate(code, lang, userQ);
          } else if (aiCurrentTab === 'review') {
            result = aiReview(code, lang, userQ);
          } else if (aiCurrentTab === 'test') {
            result = aiTest(code, lang, userQ);
          } else if (aiCurrentTab === 'refactor') {
            result = aiRefactor(code, lang, userQ);
          }
        }

        // 添加AI回复到对话历史
        var aiMsg = { role: 'ai', content: result, time: new Date().toLocaleTimeString('zh-CN', {hour: '2-digit', minute: '2-digit'}) };
        aiChatHistory.push(aiMsg);

        // 打字机流式生成效果
        typewriterAI(result, function() {
    ...
    // 打字机流式生成（支持HTML内容，随机字数+随机延迟，模拟真实AI生成）
    function typewriterAI(html, callback) {
      var div = document.createElement('div');
      div.className = 'bce-ai-msg bce-ai-msg-ai bce-ai-typing';
      var bubble = document.createElement('div');
      bubble.className = 'bce-ai-msg-bubble';
      var loading = document.createElement('span');
      loading.className = 'bce-ai-loading';
      ...
      function typeNext() {
        if (nodeIndex >= nodes.length) {
          // 生成完成
          loading.remove();
          div.classList.remove('bce-ai-typing');
          var timeEl = document.createElement('div');
          timeEl.className = 'bce-ai-msg-time';
        ...
        if (node.nodeType === 3) {
          ...
          if (charIndex < text.length) {
            ...
            var chunk = text.substring(charIndex, charIndex + chunkSize);
            bubble.insertBefore(document.createTextNode(chunk), loading);
            charIndex += chunkSize;
            aiOutputWrap.scrollTop = aiOutputWrap.scrollHeight;
            // 随机延迟10-50ms，模拟真实生成节奏
            var delay = Math.floor(Math.random() * 40) + 10;
          ...
          // 元素节点：整体插入（代码块、按钮、标题等）
          bubble.insertBefore(node.cloneNode(true), loading);
          nodeIndex++;
          aiOutputWrap.scrollTop = aiOutputWrap.scrollHeight;
          // 元素插入后随机停顿
          setTimeout(typeNext, Math.floor(Math.random() * 60) + 20);
        } else {
          nodeIndex++;
          typeNext();
        }
      ...
      // 点击气泡可跳过生成动画
      bubble.addEventListener('click', function skipTyping() {
        ...
        timeEl.textContent = new Date().toLocaleTimeString('zh-CN', {hour: '2-digit', minute: '2-digit'});
        div.appendChild(timeEl);
        bubble.removeEventListener('click', skipTyping);
        aiOutputWrap.scrollTop = aiOutputWrap.scrollHeight;
        if (callback) callback();
      });
    ...
    function renderChatMessage(msg) {
      var div = document.createElement('div');
      div.className = 'bce-ai-msg ' + (msg.role === 'user' ? 'bce-ai-msg-user' : 'bce-ai-msg-ai');
      div.innerHTML = '<div class="bce-ai-msg-bubble">' + msg.content + '</div>' +
                      '<div class="bce-ai-msg-time">' + msg.time + '</div>';
      aiOutput.appendChild(div);
    }

    // ========== 高效AI对话引擎（1000+场景，数据驱动+模板渲染+索引匹配） ==========

    // 场景数据库：通过关键词变体×回复模板组合生成1000+对话体验
    var AI_SCENARIOS = [
      // ===== 日常问候类（12场景×8回复=96组合） =====
    ...
    function checkSyntax(code, lang) {
      ...
      // 需要分号的语言
      var needSemicolon = ['javascript', 'typescript', 'java', 'c', 'cpp', 'go', 'rust'];
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        for (var j = 0; j < line.length; j++) {
          ...
          if (ch === '(' || ch === '[' || ch === '{') {
            ...
            if (stack.length === 0) {
              errors.push({ line: i + 1, col: j + 1, type: 'error', msg: '多余的闭合括号 "' + ch + '"' });
            } else {
              var top = stack.pop();
              if (top.char !== pairs[ch]) {
                errors.push({ line: i + 1, col: j + 1, type: 'error', msg: '括号不匹配：期望 "' + Object.keys(pairs).find(function(k){return pairs[k]===top.char}) + '"，实际是 "' + ch + '"' });
              }
            }
          }
        }
        ...
        if (inString && stringChar !== '`') {
          errors.push({ line: i + 1, type: 'error', msg: '字符串未闭合（缺少结束的 ' + stringChar + '）' });
          inString = false;
        }
      }
      ...
      // 未闭合的块注释
      if (inBlockComment) {
        errors.push({ line: lines.length, type: 'error', msg: '块注释未闭合（缺少 */）' });
      }

      // 未闭合的括号
      while (stack.length > 0) {
        var unclosed = stack.pop();
        errors.push({ line: unclosed.line, col: unclosed.col, type: 'error', msg: '括号 "' + unclosed.char + '" 未闭合' });
      }

      // 2. 分号缺失检测（需要分号的语言）
      if (needSemicolon.indexOf(lang) !== -1) {
        for (var k = 0; k < lines.length; k++) {
          var l = lines[k].trim();
          if (!l) continue;
          // 跳过注释、预处理指令、以{/}/(/[/,结尾的行、控制语句、函数定义、class定义
          if (/^(\/\/|#|import|package|public|private|protected|class|struct|interface|func|fn|def|if|else|for|while|switch|case|default|try|catch|finally|do|return|break|continue|throw|yield|async|await|static|const|let|var|type|impl|trait|use|extern|crate|mod|enum|union|typedef|namespace|template|typename|virtual|override|new|delete|this|super|base|init|deinit|constructor|destructor|operator|friend|inline|explicit|mutable|volatile|register|auto|sizeof|alignof|alignas|noexcept|throw|constexpr|decltype|static_assert|thread_local|export|module|import|concept|requires|co_await|co_yield|co_return|ref|out|in|params|get|set|add|remove|invoke|raise|event|delegate|using|namespace|partial|sealed|abstract|virtual|override|new|static|readonly|const|volatile|unsafe|fixed|stackalloc|checked|unchecked|lock|yield|await|async|when|where|select|from|group|into|join|let|orderby|on|equals|by|ascending|descending|var|dynamic|default|nameof|sizeof|typeof|is|as|null|true|false|value|arg|args|e|sender|eventargs|exception|message|stacktrace|innerexception|data|source|targetsite|helplink|hresult|tostring|gethashcode|gettype|equals|referenceequals|clone|compareto|getenumerator|dispose|finalize|memberwiseclone|makebyreftype|typereference|methodhandle|fieldhandle|propertymetadata|dependencyproperty|routedevent|command|binding|relaycommand|observablecollection|inotifypropertychanged|inotifycollectionchanged|icomparable|icomparer|iequatable|iequalitycomparer|icloneable|idisposable|ienumerable|ienumerator|icollection|ilist|idictionary|ireadonlylist|ireadonlycollection|ireadonlydictionary|iset|idictionaryenumerator|hashtable|arraylist|queue|stack|sortedlist|bitarray|hashtable|sorted dictionary|sortedset|hashset|lookup|grouping|lookuplist|dictionaryentry|keyvaluepair|tuple|valuetuple|nullable|lazy|interlocked|monitor|mutex|semaphore|autoresetevent|manualresetevent|countdownevent|barrier|spinlock|spinwait|waithandle|eventwaithandle|registeredwaithandle|iocp|threadpool|thread|task|taskcompletionsource|cancellationtoken|cancellationtokensource|parallel|plinq|asparallel|assequential|withdegreeofparallelism|withmergeoptions|withexecutionmode|forall|sum|average|min|max|count|longcount|aggregate|reduce|fold|scan|unfold|iterate|repeat|range|empty|singleton|concat|zip|mapi|map2|map3|iter|iteri|foldi|fold2|fold3|scan|unfold|init|create|ofarray|oflist|ofseq|ofseq|toarray|tolist|toseq|todictionary|toset|ofdict|trymax|tryfind|trypick|exists|forall|find|findindex|findback|findindexback|pick|filter|choose|partition|split|groupby|countby|sortby|sortbydescending|sort|sortdescending|rev|reverse|distinct|distinctby|head|tail|last|nth|item|get|set|iter|iteri|fold|foldi|fold2|fold3|scan|unfold|init|create|ofarray|oflist|ofseq|toarray|tolist|toseq|todictionary|toset|ofdict|trymax|tryfind|trypick|exists|forall|find|findindex|findback|findindexback|pick|filter|choose|partition|split|groupby|countby|sortby|sortbydescending|sort|sortdescending|rev|reverse|distinct|distinctby|head|tail|last|nth|item|get|set)/.test(l)) continue;
          if (/[{}()\[\],;:]$/.test(l)) continue;
          if (/^[{}()\[\]]+$/.test(l)) continue;
          // 简单检测：代码行不以分号结尾
          if (!/[;{}()\[\],:]$/.test(l) && l.length > 2 && !/^(else|do|try|finally)$/.test(l)) {
            // 进一步过滤：可能是多行语句的中间行
            var prevLine = k > 0 ? lines[k - 1].trim() : '';
            var nextLine = k < lines.length - 1 ? lines[k + 1].trim() : '';
            // 如果上一行以操作符结尾，可能是多行语句
            if (/[+\-*/%&|^<>=!?]$/.test(prevLine)) continue;
            // 如果下一行以.或操作符开头，可能是链式调用
            if (/^[.+\-*/%&|^<>=!?]/.test(nextLine)) continue;
            // Go语言的if/for不需要括号但需要大括号
            if (lang === 'go' && /^(if|for|switch|select)\s+/.test(l)) continue;
            // Rust语言的if/for/match不需要分号
            if (lang === 'rust' && /^(if|for|while|loop|match|impl|fn|struct|enum|trait|mod|use|crate|extern|pub|unsafe|async|move|static|const|type|where|dyn|impl|for|in|as|ref|mut|move|static|const|unsafe|async|await|dyn|impl|trait|fn|struct|enum|union|type|where|self|Self|super|crate|mod|use|extern|pub|priv|static|const|unsafe|async|await|dyn|impl|trait|fn|struct|enum|union|type|where|self|Self|super|crate|mod|use|extern|pub|priv|static|const|unsafe|async|await|dyn|impl|trait|fn|struct|enum|union|type|where|self|Self|super|crate|mod|use|extern|pub|priv)/.test(l)) continue;
            errors.push({ line: k + 1, type: 'warning', msg: '可能缺少分号（语句未以 ; 结尾）' });
          }
        }
      }
      ...
      if (lang === 'python') {
        ...
        for (var p = 0; p < lines.length; p++) {
          var pl = lines[p];
          if (!pl.trim() || pl.trim().startsWith('#')) continue;
          var indent = pl.length - pl.trimStart().length;
          if (indent > indentStack[indentStack.length - 1]) {
            ...
            if (indent !== indentStack[indentStack.length - 1]) {
              errors.push({ line: p + 1, type: 'error', msg: '缩进不一致（期望 ' + indentStack[indentStack.length - 1] + ' 空格，实际 ' + indent + ' 空格）' });
            }
          }
        }
      }

      // 4. 简单的未定义变量检测（仅对JS/TS）
      if (lang === 'javascript' || lang === 'typescript') {
        var definedVars = {};
        // 收集定义的变量
        code.replace(/\b(?:var|let|const|function|class)\s+(\w+)/g, function(m, name) {
          definedVars[name] = true;
          return m;
        });
        // 收集函数参数
        code.replace(/function\s+\w+\s*\(([^)]*)\)/g, function(m, params) {
          params.split(',').forEach(function(p) {
            var pname = p.trim().split(/[=:]/)[0].trim();
            if (pname) definedVars[pname] = true;
          });
          return m;
        });
        // 内置变量
        ['console', 'document', 'window', 'Math', 'JSON', 'Array', 'Object', 'String', 'Number', 'Boolean', 'Date', 'RegExp', 'Error', 'Promise', 'Map', 'Set', 'Symbol', 'BigInt', 'globalThis', 'process', 'require', 'module', 'exports', '__dirname', '__filename', 'setTimeout', 'setInterval', 'clearTimeout', 'clearInterval', 'fetch', 'alert', 'prompt', 'confirm', 'parseInt', 'parseFloat', 'isNaN', 'isFinite', 'encodeURI', 'decodeURI', 'encodeURIComponent', 'decodeURIComponent', 'escape', 'unescape', 'eval', 'undefined', 'NaN', 'Infinity', 'this', 'arguments', 'new', 'typeof', 'instanceof', 'in', 'of', 'delete', 'void', 'yield', 'await', 'async', 'super', 'extends', 'static', 'get', 'set', 'import', 'export', 'default', 'from', 'as', 'null', 'true', 'false'].forEach(function(v) { definedVars[v] = true; });
      }

      return {
        ok: errors.length === 0,
        errors: errors.filter(function(e) { return e.type === 'error'; }),
        warnings: errors.filter(function(e) { return e.type === 'warning'; }),
        all: errors
      };
    }
    ...
    function analyzeCode(code, lang) {
      ...
      if (nonEmpty.length > 0) {
        stats.avgLineLen = Math.round(nonEmpty.reduce(function (s, l) { return s + l.length; }, 0) / nonEmpty.length);
      }
      var commentRe = {
      ...
      var cre = commentRe[lang] || /^\s*(\/\/|#)/;
      nonEmpty.forEach(function (l) {
        if (cre.test(l)) stats.commentLines++;
      });
      // 函数定义
      var funcRe = /(def |function |func |fn |public\s+(static\s+)?\w+\s+\w+\s*\(|int\s+main\s*\()/;
      // 类定义
      var classRe = /(class |struct |interface |type\s+\w+\s+struct|impl |trait )/;
      // 循环
      var loopRe = /(\bfor\b|\bwhile\b|\bloop\b|\.forEach\(|\.map\(|\.filter\()/;
      // 条件
      var condRe = /(\bif\b|\belse\b|\belif\b|\bswitch\b|\bmatch\b|\?.*:)/;
      // 导入
      var importRe = /(import |#include|use |require |from\s+.*\s+import)/;
      // 变量声明
      var varRe = /(\bvar\b|\blet\b|\bconst\b|\bint\b|\bfloat\b|\bdouble\b|\bchar\b|\bstring\b|\bbool\b|\bauto\b|\bval\b|\bvar\b|:=|=\s*[^=])/;
      nonEmpty.forEach(function (l) {
        if (funcRe.test(l)) stats.functions++;
        if (classRe.test(l)) stats.classes++;
        if (loopRe.test(l)) stats.loops++;
        if (condRe.test(l)) stats.conditions++;
        if (importRe.test(l)) stats.imports++;
        if (varRe.test(l)) stats.variables++;
        if (/\breturn\b/.test(l)) stats.returns++;
        if (/(print|console\.log|printf|cout|println|fmt\.Println|println!|System\.out\.println)/.test(l)) stats.prints++;
      });
      // 计算最大嵌套深度
      ...
      nonEmpty.forEach(function(l) {
        var matches = l.match(/\b(if|else if|for|while|case|catch|\?\s*[^:]+:|&&|\|\|)/g);
        if (matches) decisionPoints += matches.length;
      });
      stats.cyclomaticComplexity = 1 + decisionPoints;

      // 代码质量评分（0-100）
      var qualityScore = 100;
      if (stats.maxNesting > 4) qualityScore -= (stats.maxNesting - 4) * 10;
      if (stats.cyclomaticComplexity > 10) qualityScore -= (stats.cyclomaticComplexity - 10) * 5;
      if (stats.avgLineLen > 100) qualityScore -= 10;
      if (stats.codeLines > 100 && stats.functions === 0) qualityScore -= 15;
      if (stats.commentLines === 0 && stats.codeLines > 20) qualityScore -= 10;
      qualityScore = Math.max(0, Math.min(100, qualityScore));
      stats.qualityScore = qualityScore;
      stats.qualityLevel = qualityScore >= 80 ? '优秀' : qualityScore >= 60 ? '良好' : qualityScore >= 40 ? '一般' : '较差';

      // 性能问题检测
      stats.perfIssues = [];
      if (/for\s*\([^)]*\.length/.test(code)) stats.perfIssues.push('循环中重复访问 .length，建议提前缓存');
      if (/\.forEach\s*\([^)]*\)\s*\{[^}]*\.push/.test(code)) stats.perfIssues.push('forEach中push可考虑用map替代');
      if (/new\s+(Array|Object|String|Number|Boolean|Date|RegExp)/.test(code)) stats.perfIssues.push('避免使用包装对象构造函数，用字面量更高效');
      if (/try\s*\{[^}]*\}\s*catch/.test(code) && /for\s*\(/.test(code)) stats.perfIssues.push('try-catch在循环中可能影响性能，考虑移出循环');

      // 安全问题检测
      stats.securityIssues = [];
      if (/eval\s*\(/.test(code)) stats.securityIssues.push('使用 eval() 存在代码注入风险');
      if (/innerHTML\s*=/.test(code)) stats.securityIssues.push('直接设置 innerHTML 存在 XSS 风险，建议用 textContent 或转义');
      if (/document\.write\s*\(/.test(code)) stats.securityIssues.push('document.write() 已废弃，存在安全风险');
      if (/setTimeout\s*\(\s*["']/.test(code) || /setInterval\s*\(\s*["']/.test(code)) stats.securityIssues.push('字符串形式的 setTimeout/setInterval 存在注入风险');
      if (/localStorage\s*\.\s*(setItem|getItem)/.test(code)) stats.securityIssues.push('localStorage 存储敏感数据存在安全风险');

      // 代码异味检测
      stats.codeSmells = [];
      if (stats.functions > 0) {
        var funcLines = code.split(/\b(function|def|func|fn)\b/);
        funcLines.forEach(function(part) {
          if (part.split('\n').length > 50) stats.codeSmells.push('存在超过50行的函数，建议拆分');
        });
      }
      if (/TODO|FIXME|HACK|XXX/.test(code)) stats.codeSmells.push('代码中存在 TODO/FIXME 等待办标记');
      if (/console\.log\s*\(/.test(code) && stats.codeLines > 30) stats.codeSmells.push('生产代码中建议移除 console.log 调试语句');

      return stats;
    }

    function getLangCommentStyle(lang) {
      var styles = {
        python: { line: '#', block: '"""', doc: 'docstring' },
        javascript: { line: '//', block: '/* */', doc: 'JSDoc' },
        typescript: { line: '//', block: '/* */', doc: 'JSDoc' },
        java: { line: '//', block: '/* */', doc: 'JavaDoc' },
        c: { line: '//', block: '/* */', doc: 'comment' },
        cpp: { line: '//', block: '/* */', doc: 'comment' },
        go: { line: '//', block: '/* */', doc: 'GoDoc' },
        rust: { line: '//', block: '/* */', doc: 'rustdoc' }
      };
      return styles[lang] || styles.javascript;
    }

    function bindAIApplyButtons() {
      var btns = aiOutput.querySelectorAll('.bce-ai-apply');
      ...
      // 预设生成按钮
      var presetBtns = aiOutput.querySelectorAll('.bce-ai-gen-preset');
    ...
    function aiExplain(code, lang, q) {
      var stats = analyzeCode(code, lang);
      var langName = CL.Languages.get(lang).name;
      var html = '<h4>代码解释</h4>';

      // 代码概览统计
      html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 14px;margin:10px 0;padding:10px 12px;background:var(--bg-soft);border-radius:var(--radius-sm);font-size:12.5px">';
      html += '<span>语言：<strong>' + esc(langName) + '</strong></span>';
      html += '<span>总行数：<strong>' + stats.totalLines + '</strong></span>';
      html += '<span>代码行：<strong>' + stats.codeLines + '</strong></span>';
      html += '<span>注释行：<strong>' + stats.commentLines + '</strong></span>';
      html += '<span>函数：<strong>' + stats.functions + '</strong></span>';
      html += '<span>类/结构：<strong>' + stats.classes + '</strong></span>';
      html += '<span>循环：<strong>' + stats.loops + '</strong></span>';
      html += '<span>条件判断：<strong>' + stats.conditions + '</strong></span>';
      html += '<span>最大嵌套：<strong>' + stats.maxNesting + ' 层</strong></span>';
      html += '<span>平均行宽：<strong>' + stats.avgLineLen + ' 字符</strong></span>';
      html += '</div>';

      // 功能概述
      html += '<p style="margin:12px 0 6px;font-weight:600">功能概述</p><ul>';
      var features = [];
      if (stats.imports > 0) features.push('导入了 ' + stats.imports + ' 个依赖库/模块');
      if (stats.classes > 0) features.push('定义了 ' + stats.classes + ' 个类/数据结构');
      if (stats.functions > 0) features.push('定义了 ' + stats.functions + ' 个函数/方法');
      if (stats.loops > 0) features.push('包含 ' + stats.loops + ' 处循环结构');
      if (stats.conditions > 0) features.push('包含 ' + stats.conditions + ' 处条件判断');
      if (stats.prints > 0) features.push('有 ' + stats.prints + ' 处控制台输出');
      if (stats.returns > 0) features.push('有 ' + stats.returns + ' 处返回语句');
      if (features.length === 0) features.push('代码结构较简单，主要为顺序执行');
      features.forEach(function (f) { html += '<li>' + f + '</li>'; });
      html += '</ul>';
      ...
      lines.forEach(function (line, i) {
        ...
        if (!t) return;
        var desc = '';
        if (/(import |#include|use |require |from\s+.*\s+import)/.test(t)) desc = '导入依赖模块';
        else if (/(class |struct |interface |type\s+\w+\s+struct|impl |trait )/.test(t)) desc = '定义类/数据结构';
        else if (/(def |function |func |fn |public\s+(static\s+)?\w+\s+\w+\s*\(|int\s+main\s*\()/.test(t)) desc = '定义函数/方法入口';
        else if (/(\bfor\b|\bwhile\b|\bloop\b)/.test(t)) desc = '循环开始';
        else if (/(\bif\b|\belse if\b|\belif\b|\bswitch\b|\bmatch\b)/.test(t)) desc = '条件判断';
        else if (/\breturn\b/.test(t)) desc = '返回结果';
        else if (/(print|console\.log|printf|cout|println|fmt\.Println|println!|System\.out\.println)/.test(t)) desc = '输出到控制台';
        else if (/(var |let |const |int |float |double |char |string |bool |auto |val |:=)/.test(t)) desc = '声明变量';
        if (desc && keyLines.length < 8) {
          keyLines.push({ line: i + 1, code: t.substring(0, 60), desc: desc });
        }
      });
      if (keyLines.length > 0) {
        html += '<p style="margin:12px 0 6px;font-weight:600">关键代码行</p>';
        html += '<div style="font-size:12px;font-family:var(--font-mono)">';
        keyLines.forEach(function (k) {
          html += '<div style="padding:4px 0;border-bottom:1px solid var(--line-soft)">';
          html += '<span style="color:var(--fg-mute)">L' + k.line + '</span> ';
          html += '<span style="color:var(--code-fg)">' + esc(k.code) + '</span>';
          html += '<div style="color:var(--brand-600);margin-left:28px;font-family:var(--font-sans);font-size:12px">→ ' + k.desc + '</div>';
          html += '</div>';
        });
        html += '</div>';
      }
      ...
      // 复杂度估算
      html += '<p style="margin:12px 0 6px;font-weight:600">复杂度估算</p>';
      var complexity = 'O(n)';
      if (stats.loops === 0) complexity = 'O(1)';
      else if (stats.loops === 1) complexity = 'O(n)';
      else if (stats.loops >= 2) complexity = 'O(n²) 或更高（需检查是否嵌套）';
      html += '<p style="font-size:13px">时间复杂度约为 <strong>' + complexity + '</strong>，';
      html += '空间复杂度约为 <strong>O(' + (stats.variables > 10 ? 'n' : '1') + ')</strong>。';
      if (stats.maxNesting > 3) html += ' 代码嵌套较深（' + stats.maxNesting + ' 层），建议拆分函数降低复杂度。';
      html += '</p>';

      if (q) {
        html += '<div style="margin-top:12px;padding:10px 12px;background:var(--brand-50);border-left:3px solid var(--brand-500);border-radius:0 var(--radius-sm) var(--radius-sm) 0;font-size:13px">';
        html += '<strong>关于你的问题：</strong>' + esc(q) + '<br>';
        html += '<span style="color:var(--fg-mute)">建议查看教程中相关语法章节，或在代码中添加更多上下文以便更精准分析。</span>';
        html += '</div>';
      }

      // 添加注释版本的应用按钮
      try {
        var commentedCode = aiComment(code, lang);
        // 提取aiComment返回的代码部分（aiComment返回HTML，需要提取<pre>中的代码）
        var codeMatch = commentedCode.match(/<code>([\s\S]*?)<\/code>/);
        if (codeMatch) {
          var plainCode = codeMatch[1].replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#39;/g, "'");
          html += '<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">';
          html += '<p style="font-size:13px;margin-bottom:8px">需要带注释的版本？一键应用到编辑器：</p>';
          html += '<button class="bce-btn bce-btn-sm bce-ai-apply" data-code="' + encodeURIComponent(plainCode) + '">应用带注释版本</button>';
          html += '</div>';
        }
      } catch (e) {}
    ...
    function aiOptimize(code, lang) {
      ...
      if (stats.totalLines > 50) {
        suggestions.push({ level: 'mid', text: '代码超过 50 行，建议拆分为多个函数，每个函数职责单一' });
      } else if (stats.totalLines > 30) {
        suggestions.push({ level: 'low', text: '代码较长，可考虑将独立逻辑抽取为函数' });
      }

      // 2. 嵌套深度
      if (stats.maxNesting > 4) {
        suggestions.push({ level: 'high', text: '嵌套深度达 ' + stats.maxNesting + ' 层，严重影响可读性，建议提前返回（guard clause）或拆分函数' });
      } else if (stats.maxNesting > 3) {
        suggestions.push({ level: 'mid', text: '嵌套较深（' + stats.maxNesting + ' 层），可考虑用提前返回减少嵌套' });
      }

      // 3. JS/TS 特定优化
      if (lang === 'javascript' || lang === 'typescript') {
        if (/\bvar\s+\w+/.test(code)) {
          suggestions.push({ level: 'high', text: '使用了 <code>var</code>，建议替换为 <code>const</code>（不重新赋值）或 <code>let</code>（重新赋值），避免变量提升问题' });
          optimized = optimized.replace(/\bvar\s+/g, 'let ');
          hasOptimization = true;
        }
        if (/console\.log\s*\([^)]*\+/.test(code)) {
          suggestions.push({ level: 'low', text: '字符串拼接使用 <code>+</code>，建议改用模板字符串 <code>`${var}`</code> 提升可读性' });
        }
        if (/==[^=]/.test(code)) {
          suggestions.push({ level: 'high', text: '使用了宽松相等 <code>==</code>，建议改用严格相等 <code>===</code> 避免类型转换陷阱' });
          optimized = optimized.replace(/==([^=])/g, '===$1');
          hasOptimization = true;
        }
        if (/!=[^=]/.test(code)) {
          suggestions.push({ level: 'high', text: '使用了宽松不等 <code>!=</code>，建议改用严格不等 <code>!==</code>' });
          optimized = optimized.replace(/!=([^=])/g, '!==$1');
          hasOptimization = true;
        }
        // 移除多余的分号
      ...
      if (lang === 'python') {
        if (/range\s*\(\s*len\s*\(/.test(code)) {
          suggestions.push({ level: 'mid', text: '使用 <code>range(len(x))</code> 遍历索引，建议直接用 <code>for item in x:</code> 或 <code>enumerate(x)</code>' });
        }
        if (/^\s*[a-z_]+\s*=\s*\[\s*\]$/m.test(code) && /\.append\s*\(/.test(code)) {
          suggestions.push({ level: 'low', text: '循环中用 append 构建列表，建议改用列表推导式 <code>[x for x in ...]</code> 更高效' });
        }
        if (/==\s*None|!=\s*None/.test(code)) {
          suggestions.push({ level: 'high', text: '用 <code>== None</code> 比较，建议改用 <code>is None</code>（Python惯用写法）' });
          optimized = optimized.replace(/==\s*None/g, 'is None').replace(/!=\s*None/g, 'is not None');
          hasOptimization = true;
        }
        if (/print\s*\([^)]*\+/.test(code)) {
          suggestions.push({ level: 'low', text: '字符串拼接用 <code>+</code>，建议改用 f-string <code>f"{var}"</code>（Python 3.6+）' });
        }
      }

      // 3.6 Java 特定优化
      if (lang === 'java') {
        if (/new\s+String\s*\(\s*["']/.test(code)) {
          suggestions.push({ level: 'high', text: '用 <code>new String("...")</code> 创建字符串，建议直接用字符串字面量，避免多余对象创建' });
          optimized = optimized.replace(/new\s+String\s*\(\s*(["'][^"']*["'])\s*\)/g, '$1');
          hasOptimization = true;
        }
        if (/String\s+\w+\s*=\s*""\s*;/.test(code) && /\+\s*=/.test(code)) {
          suggestions.push({ level: 'mid', text: '循环中用 String 拼接，建议改用 <code>StringBuilder</code> 提升性能' });
        }
        if (/==\s*["']|["']\s*==/.test(code)) {
          suggestions.push({ level: 'high', text: '用 <code>==</code> 比较字符串，建议改用 <code>.equals()</code> 避免引用比较错误' });
        }
      }

      // 3.7 C/C++ 特定优化
      if (lang === 'c' || lang === 'cpp') {
        if (/printf\s*\(\s*["'][^"']*\\n["']\s*\)/.test(code) && lang === 'cpp') {
          suggestions.push({ level: 'low', text: 'C++ 中用 printf，建议改用 <code>std::cout</code> 更符合 C++ 风格' });
        }
        if (/using\s+namespace\s+std/.test(code)) {
          suggestions.push({ level: 'mid', text: '使用 <code>using namespace std;</code> 可能导致命名冲突，建议只 using 具体需要的名称' });
        }
        if (/malloc\s*\(/.test(code) && lang === 'cpp') {
          suggestions.push({ level: 'high', text: 'C++ 中用 malloc，建议改用 <code>new</code> 或智能指针（RAII）' });
        }
        if (/\bi\s*\+\+\b/.test(code) && lang === 'cpp') {
          suggestions.push({ level: 'low', text: '循环中用后置 <code>i++</code>，建议改用前置 <code>++i</code>（对迭代器更高效）' });
        }
      }

      // 3.8 Go 特定优化
      if (lang === 'go') {
        if (/fmt\.Sprintf\s*\(/.test(code) && /\+/.test(code)) {
          suggestions.push({ level: 'low', text: '简单字符串拼接建议用 <code>+</code>，复杂格式化才用 fmt.Sprintf' });
        }
        if (/errors\.New\s*\(\s*["']/.test(code)) {
          suggestions.push({ level: 'low', text: '可用 <code>fmt.Errorf</code> 替代 errors.New + fmt.Sprintf 组合' });
        }
      }

      // 3.9 Rust 特定优化
      if (lang === 'rust') {
        if (/\.unwrap\s*\(\)/.test(code)) {
          suggestions.push({ level: 'high', text: '使用 <code>.unwrap()</code> 可能导致 panic，建议用 <code>match</code> 或 <code>?</code> 操作符处理错误' });
        }
        if (/String::from\s*\(\s*["']/.test(code)) {
          suggestions.push({ level: 'low', text: '可用 <code>.to_string()</code> 或 <code>.into()</code> 替代 String::from' });
        }
        if (/let\s+mut\s+\w+\s*=\s*String::new/.test(code) && /push_str/.test(code)) {
          suggestions.push({ level: 'low', text: '循环中用 push_str 构建字符串，建议用 <code>format!</code> 或迭代器 collect' });
        }
      }

      // 4. 循环优化
      if (/for\s*\([^)]*\.length/.test(code)) {
        suggestions.push({ level: 'mid', text: '循环条件中重复访问 <code>.length</code>，建议提前缓存到变量：<code>for (var i = 0, len = arr.length; i < len; i++)</code>' });
      }

      // 5. 空 catch
      if (/catch\s*\([^)]*\)\s*\{\s*\}/.test(code)) {
        suggestions.push({ level: 'high', text: '存在空的 <code>catch</code> 块，会吞掉错误，建议至少记录日志或重新抛出' });
      }

      // 6. 未使用的导入（简单检测）
      if (stats.imports > 0) {
        var importMatches = code.match(/(?:import|#include|use)\s+([\w.]+)/g) || [];
        importMatches.forEach(function (imp) {
          var name = imp.replace(/.*?([\w]+)\s*[;}]?$/, '$1');
          if (name && name.length > 2) {
            var count = (code.match(new RegExp('\\b' + name + '\\b', 'g')) || []).length;
            if (count <= 1) {
              suggestions.push({ level: 'low', text: '导入的 <code>' + esc(name) + '</code> 可能未被使用，建议移除' });
            }
          }
        });
      }

      // 7. 通用建议（根据语言不同给出不同的命名规范）
      if (stats.commentLines === 0 && stats.codeLines > 10) {
        suggestions.push({ level: 'low', text: '代码无注释，建议为复杂逻辑添加注释提高可维护性' });
      }
      // 根据语言给出不同的命名规范建议
      var namingAdvice = '';
      if (lang === 'python') {
        namingAdvice = '遵循 Python 命名规范：变量/函数用蛇形命名（snake_case），常量用大写下划线，类名用大驼峰（PascalCase）';
      } else if (lang === 'javascript' || lang === 'typescript') {
        namingAdvice = '遵循 JS/TS 命名规范：变量/函数用小驼峰（camelCase），常量用大写下划线，类/接口/类型用大驼峰（PascalCase）';
      } else if (lang === 'java') {
        namingAdvice = '遵循 Java 命名规范：变量/方法用小驼峰（camelCase），常量用大写下划线，类/接口用大驼峰（PascalCase），包名全小写';
      } else if (lang === 'c' || lang === 'cpp') {
        namingAdvice = '遵循 C/C++ 命名规范：变量/函数常用小驼峰或蛇形，常量用大写下划线，类/结构体用大驼峰（PascalCase）';
      } else if (lang === 'go') {
        namingAdvice = '遵循 Go 命名规范：变量/函数用小驼峰（camelCase），导出标识符首字母大写，常量常用大驼峰或全大写，结构体用大驼峰';
      } else if (lang === 'rust') {
        namingAdvice = '遵循 Rust 命名规范：变量/函数用蛇形命名（snake_case），常量/静态变量用大写下划线，结构体/枚举/特征用大驼峰（PascalCase）';
      } else {
        namingAdvice = '遵循语言命名规范：变量用小驼峰，常量用大写下划线，类名用大驼峰';
      }
      suggestions.push({ level: 'low', text: namingAdvice });

      // 根据语言给出不同的函数参数建议
      var paramAdvice = '';
      if (lang === 'javascript' || lang === 'typescript') {
        paramAdvice = '函数参数建议不超过 3 个，过多时考虑用选项对象（options object）传递';
      } else if (lang === 'python') {
        paramAdvice = '函数参数建议不超过 3 个，过多时考虑用 *args/**kwargs 或字典/数据类传递';
      } else if (lang === 'java') {
        paramAdvice = '函数参数建议不超过 3 个，过多时考虑用对象/POJO 传递，或使用建造者模式';
      } else if (lang === 'go') {
        paramAdvice = '函数参数建议不超过 3 个，过多时考虑用结构体（struct）传递，或使用选项模式（Functional Options）';
      } else if (lang === 'rust') {
        paramAdvice = '函数参数建议不超过 3 个，过多时考虑用结构体（struct）传递，或使用元组';
      } else {
        paramAdvice = '函数参数建议不超过 3 个，过多时考虑用对象/结构体传递';
      }
      suggestions.push({ level: 'low', text: paramAdvice });

      // 输出建议
      if (suggestions.length > 0) {
        var high = suggestions.filter(function (s) { return s.level === 'high'; });
        var mid = suggestions.filter(function (s) { return s.level === 'mid'; });
        var low = suggestions.filter(function (s) { return s.level === 'low'; });
        html += '<p>共发现 <strong>' + suggestions.length + '</strong> 条优化建议（高优先级 ' + high.length + '，中 ' + mid.length + '，低 ' + low.length + '）：</p>';
        html += '<ul style="margin:8px 0;padding-left:18px">';
        high.forEach(function (s) {
          html += '<li style="margin:4px 0"><span style="color:#ef4444;font-weight:600">[高]</span> ' + s.text + '</li>';
        });
        mid.forEach(function (s) {
          html += '<li style="margin:4px 0"><span style="color:#f59e0b;font-weight:600">[中]</span> ' + s.text + '</li>';
        });
        low.forEach(function (s) {
          html += '<li style="margin:4px 0"><span style="color:#10b981;font-weight:600">[低]</span> ' + s.text + '</li>';
        });
        html += '</ul>';
      }
      ...
      // 优化后的代码对比
      if (hasOptimization) {
        html += '<p style="margin:12px 0 6px;font-weight:600">自动优化后的代码</p>';
        html += '<pre class="bce-ai-code"><code>' + esc(optimized) + '</code></pre>';
        html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:8px" data-code="' + encodeURIComponent(optimized) + '">应用优化后的代码</button>';
      } else {
        // 即使没有自动优化，也提供格式化版本（基本清理：移除行尾空格、合并多余空行）
        var formatted = code
          .replace(/[ \t]+$/gm, '')           // 移除行尾空格
          .replace(/\n{3,}/g, '\n\n')         // 合并连续空行（最多2个）
          .replace(/^\n+|\n+$/g, '');         // 移除开头和结尾的空行
        if (formatted !== code) {
          html += '<p style="margin:12px 0 6px;font-weight:600">格式化后的代码</p>';
          html += '<pre class="bce-ai-code"><code>' + esc(formatted) + '</code></pre>';
          html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:8px" data-code="' + encodeURIComponent(formatted) + '">应用格式化版本</button>';
        } else {
          // 代码已经很规范，也提供应用按钮（至少让用户可以重新应用）
          html += '<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">';
          html += '<p style="font-size:13px;margin-bottom:8px">代码已较为规范，可点击下方按钮重新应用到编辑器：</p>';
          html += '<button class="bce-btn bce-btn-sm bce-ai-apply" data-code="' + encodeURIComponent(code) + '">重新应用当前代码</button>';
          html += '</div>';
        }
      }

      html += '<p style="margin-top:12px;color:var(--fg-mute);font-size:12px">以上为静态分析建议，具体优化需结合实际业务场景和性能测试。</p>';
      return html;
    }
    function aiComment(code, lang) {
      var style = getLangCommentStyle(lang);
      var lines = code.split('\n');
      var result = [];
      var inBlock = false;
      ...
      lines.forEach(function (line, i) {
        ...
        // 跳过已有注释
        if (trimmed && (trimmed.indexOf(style.line) === 0 || trimmed.indexOf('/*') === 0 || trimmed.indexOf('*') === 0 || trimmed.indexOf('"""') === 0)) {
          result.push(line);
          return;
        }
        ...
        // 函数/方法定义 - 添加文档注释
        var isFunc = /(def |function |func |fn |public\s+(static\s+)?\w+\s+\w+\s*\(|int\s+main\s*\()/.test(trimmed);
        if (isFunc) {
          // 提取函数名
          var funcName = 'unknown';
          var nameMatch = trimmed.match(/(?:def|function|func|fn|public\s+(?:static\s+)?\w+)\s+(\w+)/);
          if (nameMatch) funcName = nameMatch[1];
          else if (/int\s+main/.test(trimmed)) funcName = 'main';

          // 提取参数
          var params = [];
          var paramMatch = trimmed.match(/\(([^)]*)\)/);
          if (paramMatch && paramMatch[1].trim()) {
            params = paramMatch[1].split(',').map(function (p) {
              return p.trim().split(/\s+/).pop().replace(/[:=].*$/, '');
            }).filter(function (p) { return p && p !== 'void'; });
          }

          if (lang === 'python') {
            result.push(indent + '"""');
            result.push(indent + funcName + ' 函数。');
            if (params.length > 0) {
              result.push(indent + '');
              params.forEach(function (p) {
                result.push(indent + '参数:');
                result.push(indent + '    ' + p + ': 输入参数');
              });
            }
            result.push(indent + '返回:');
            result.push(indent + '    函数执行结果');
            result.push(indent + '"""');
          } else {
            result.push(indent + style.line + ' ' + funcName + ' - 函数功能描述');
            if (params.length > 0) {
              params.forEach(function (p) {
                result.push(indent + style.line + ' @param ' + p + ' 输入参数');
              });
            }
            result.push(indent + style.line + ' @return 函数执行结果');
          }
          result.push(line);
          return;
        }
        ...
        // 类/结构定义
        if (/(class |struct |interface |type\s+\w+\s+struct|impl |trait )/.test(trimmed)) {
          var className = 'unknown';
          var classMatch = trimmed.match(/(?:class|struct|interface|type|impl|trait)\s+(\w+)/);
          if (classMatch) className = classMatch[1];
          if (lang === 'python') {
            result.push(indent + '"""');
            result.push(indent + className + ' 类。');
            result.push(indent + '');
            result.push(indent + '属性:');
            result.push(indent + '    描述类的属性');
            result.push(indent + '"""');
          } else {
            result.push(indent + style.line + ' ' + className + ' - 类/数据结构描述');
          }
          result.push(line);
          return;
        }
        ...
        // 导入语句
        if (/(import |#include|use |require |from\s+.*\s+import)/.test(trimmed)) {
          result.push(line + style.line === '#' ? '  # 导入依赖模块' : '  // 导入依赖模块');
          return;
        }

        // 循环
        if (/(\bfor\b|\bwhile\b|\bloop\b)/.test(trimmed)) {
          result.push(line + (style.line === '#' ? '  # 循环遍历' : '  // 循环遍历'));
          return;
        }

        // 条件判断
        if (/(\bif\b|\belse if\b|\belif\b|\bswitch\b|\bmatch\b)/.test(trimmed)) {
          result.push(line + (style.line === '#' ? '  # 条件判断' : '  // 条件判断'));
          return;
        }

        // 返回
        if (/\breturn\b/.test(trimmed)) {
          result.push(line + (style.line === '#' ? '  # 返回结果' : '  // 返回结果'));
          return;
        }

        // 输出
        if (/(print|console\.log|printf|cout|println|fmt\.Println|println!|System\.out\.println)/.test(trimmed)) {
          result.push(line + (style.line === '#' ? '  # 输出到控制台' : '  // 输出到控制台'));
          return;
        }

        // 变量声明
        if (/(\bvar\b|\blet\b|\bconst\b|\bint\b|\bfloat\b|\bdouble\b|\bchar\b|\bstring\b|\bbool\b|\bauto\b|\bval\b|:=)/.test(trimmed)) {
          result.push(line + (style.line === '#' ? '  # 声明变量' : '  // 声明变量'));
          return;
        }
      ...
      var commentedCode = result.join('\n');
      var html = '<h4>添加注释</h4>';
      html += '<p>已为代码添加智能注释，包括：</p>';
      html += '<ul style="margin:6px 0 10px;padding-left:18px;font-size:13px">';
      html += '<li>函数/方法的文档注释（' + style.doc + ' 格式）</li>';
      html += '<li>类/数据结构的描述注释</li>';
      html += '<li>关键代码行的行内注释</li>';
      html += '</ul>';
      html += '<pre class="bce-ai-code"><code>' + esc(commentedCode) + '</code></pre>';
      html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:10px" data-code="' + encodeURIComponent(commentedCode) + '">应用到编辑器</button>';
      return html;
    }
    function aiFix(code, lang) {
      var stats = analyzeCode(code, lang);
      var issues = [];
      var fixed = code;
      var hasFix = false;
      ...
      // 1. 括号匹配检查
      var curlyOpen = (code.match(/{/g) || []).length;
      var curlyClose = (code.match(/}/g) || []).length;
      if (curlyOpen !== curlyClose) {
        issues.push({ level: 'high', type: '语法', text: '花括号不匹配：有 ' + curlyOpen + ' 个 <code>{</code> 但只有 ' + curlyClose + ' 个 <code>}</code>', fix: false });
      }
      var parenOpen = (code.match(/\(/g) || []).length;
      var parenClose = (code.match(/\)/g) || []).length;
      if (parenOpen !== parenClose) {
        issues.push({ level: 'high', type: '语法', text: '圆括号不匹配：有 ' + parenOpen + ' 个 <code>(</code> 但只有 ' + parenClose + ' 个 <code>)</code>', fix: false });
      }
      var bracketOpen = (code.match(/\[/g) || []).length;
      var bracketClose = (code.match(/\]/g) || []).length;
      if (bracketOpen !== bracketClose) {
        issues.push({ level: 'high', type: '语法', text: '方括号不匹配：有 ' + bracketOpen + ' 个 <code>[</code> 但只有 ' + bracketClose + ' 个 <code>]</code>', fix: false });
      }

      // 2. 分号检查（C-like语言）
      if (/java|c|cpp|go|rust|javascript|typescript/.test(lang)) {
        var lines = code.split('\n');
        var missingSemicolons = [];
        lines.forEach(function (line, i) {
          var t = line.trim();
          if (!t) return;
          if (/^(\/\/|#|\/\*|\*|import|#include|use |package|@)/.test(t)) return;
          if (/[{};:,]$/.test(t)) return;
          if (/^(if|else|for|while|switch|try|catch|finally|func|fn|def|class|struct|interface|impl|trait|namespace|template|public|private|protected|static|virtual|override|inline|extern|const|let|var|auto|return|break|continue|goto|throw|new|delete|sizeof|typeof|instanceof|in|of|as|is|await|yield|async|from|with|assert|pass|lambda|global|nonlocal|raise|del|import|from)\b/.test(t)) return;
          if (t.length > 5 && !/[{}();:,\[\]]$/.test(t) && !/^[a-zA-Z_$][\w$]*\s*$/.test(t)) {
            missingSemicolons.push(i + 1);
          }
        });
        if (missingSemicolons.length > 0 && missingSemicolons.length <= 3) {
          issues.push({ level: 'mid', type: '语法', text: '第 ' + missingSemicolons.join(', ') + ' 行可能缺少分号 <code>;</code>', fix: false });
        }
      }
      ...
      if (lang === 'javascript' || lang === 'typescript') {
        // = vs ==
        if (/if\s*\([^=!<>]*=[^=]/.test(code)) {
          issues.push({ level: 'high', type: '逻辑', text: 'if 条件中可能使用了赋值 <code>=</code> 而非比较 <code>===</code>', fix: false });
        }
        // var 声明
        if (/\bvar\s+\w+/.test(code)) {
          issues.push({ level: 'mid', type: '最佳实践', text: '使用了 <code>var</code>，存在变量提升问题，建议改用 <code>let</code>/<code>const</code>', fix: true });
          fixed = fixed.replace(/\bvar\s+/g, 'let ');
          hasFix = true;
        }
        // == vs ===
        if (/==[^=]/.test(code)) {
          issues.push({ level: 'high', type: '逻辑', text: '使用了宽松相等 <code>==</code>，可能导致类型转换错误，建议改用 <code>===</code>', fix: true });
      ...
      if (lang === 'python') {
        ...
        var indentIssues = [];
        pyLines.forEach(function (line, i) {
          if (!line.trim()) return;
          var spaces = line.match(/^ */)[0].length;
          if (spaces % 4 !== 0 && spaces > 0) {
            indentIssues.push(i + 1);
          }
        });
        if (indentIssues.length > 0 && indentIssues.length <= 5) {
          issues.push({ level: 'mid', type: '语法', text: '第 ' + indentIssues.join(', ') + ' 行缩进不是 4 的倍数，Python 要求一致的缩进', fix: false });
        }
        // 冒号检查
        if (/\b(def|if|elif|else|for|while|class|try|except|finally|with)\b[^\n:]*$/.test(code)) {
          issues.push({ level: 'high', type: '语法', text: '控制语句可能缺少冒号 <code>:</code>', fix: false });
        }
      }

      // 5. 空 catch
      if (/catch\s*\([^)]*\)\s*\{\s*\}/.test(code)) {
        issues.push({ level: 'high', type: '逻辑', text: '存在空的 <code>catch</code> 块，会吞掉异常，建议至少记录错误日志', fix: false });
      }

      // 6. 未使用的变量（简单检测）
      if (stats.variables > 0) {
        var varMatches = code.match(/\b(?:var|let|const|int|float|double|char|string|bool|auto|val)\s+(\w+)/g) || [];
        var unusedVars = [];
        varMatches.forEach(function (v) {
          var name = v.replace(/.*?(\w+)\s*[=;]?$/, '$1');
          if (name && name.length > 1 && name !== 'main') {
            var count = (code.match(new RegExp('\\b' + name + '\\b', 'g')) || []).length;
            if (count <= 1) unusedVars.push(name);
          }
        });
        if (unusedVars.length > 0 && unusedVars.length <= 5) {
          issues.push({ level: 'low', type: '最佳实践', text: '变量 <code>' + unusedVars.join(', ') + '</code> 声明后可能未被使用', fix: false });
        }
      }

      // 7. 硬编码值
      if (/\b(?:var|let|const|int|float|double)\s+\w+\s*=\s*["'\d]/.test(code) && stats.codeLines > 10) {
        issues.push({ level: 'low', type: '最佳实践', text: '存在硬编码值，建议提取为常量或配置', fix: false });
      }

      // 8. console.log 残留
      if (/console\.log/.test(code) && stats.codeLines > 20) {
        issues.push({ level: 'low', type: '最佳实践', text: '代码中包含 <code>console.log</code>，生产环境建议移除或改用日志库', fix: false });
      }

      // 输出结果
      var html = '<h4>Bug 检测</h4>';
      if (issues.length === 0) {
        html += '<div style="padding:14px;background:var(--brand-50);border-radius:var(--radius-sm);text-align:center">';
        html += '<div style="font-size:32px;margin-bottom:6px">✓</div>';
        html += '<p style="font-weight:600;color:var(--brand-700);margin:0">未检测到明显问题</p>';
        html += '<p style="color:var(--fg-mute);font-size:12px;margin:4px 0 0">代码结构良好，建议点击「运行」进行实际测试</p>';
        html += '</div>';
      } else {
        var high = issues.filter(function (i) { return i.level === 'high'; });
        var mid = issues.filter(function (i) { return i.level === 'mid'; });
        var low = issues.filter(function (i) { return i.level === 'low'; });
        html += '<p>共检测到 <strong>' + issues.length + '</strong> 个问题（严重 ' + high.length + '，中等 ' + mid.length + '，轻微 ' + low.length + '）：</p>';
        html += '<div style="margin:8px 0">';
        issues.forEach(function (iss) {
          var color = iss.level === 'high' ? '#ef4444' : iss.level === 'mid' ? '#f59e0b' : '#10b981';
          var label = iss.level === 'high' ? '严重' : iss.level === 'mid' ? '中等' : '轻微';
          html += '<div style="padding:8px 10px;margin:4px 0;background:var(--bg-soft);border-left:3px solid ' + color + ';border-radius:0 var(--radius-xs) var(--radius-xs) 0">';
          html += '<div style="font-size:12px;margin-bottom:2px"><span style="color:' + color + ';font-weight:600">[' + label + ']</span> <span style="color:var(--fg-mute)">' + iss.type + '</span></div>';
          html += '<div style="font-size:13px">' + iss.text + '</div>';
          html += '</div>';
        });
        html += '</div>';
      }
      ...
      // 自动修复后的代码
      if (hasFix) {
        html += '<p style="margin:12px 0 6px;font-weight:600">自动修复后的代码</p>';
        html += '<p style="font-size:12px;color:var(--fg-mute);margin-bottom:6px">已自动修复可安全修改的问题（如 var→let、==→===），其他问题需手动确认：</p>';
        html += '<pre class="bce-ai-code"><code>' + esc(fixed) + '</code></pre>';
        html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:8px" data-code="' + encodeURIComponent(fixed) + '">应用修复后的代码</button>';
      } else {
        // 即使没有可自动修复的问题，也提供应用按钮
        html += '<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">';
        html += '<p style="font-size:13px;margin-bottom:8px">未检测到可自动修复的问题，可点击下方按钮重新应用当前代码：</p>';
        html += '<button class="bce-btn bce-btn-sm bce-ai-apply" data-code="' + encodeURIComponent(code) + '">重新应用当前代码</button>';
        html += '</div>';
      }

      html += '<p style="margin-top:12px;color:var(--fg-mute);font-size:12px">静态分析无法检测所有运行时错误，建议结合实际运行测试。</p>';
      return html;
    }

    /* ---------- AI 代码生成 ---------- */
    function aiGenerate(code, lang, desc) {
      var html = '<h4>代码生成</h4>';
      if (!desc || desc.length < 2) {
        html += '<div style="padding:16px;text-align:center;color:var(--fg-mute)">';
        html += '<p style="font-size:14px;margin-bottom:10px">请在上方输入框描述你想要生成的代码功能</p>';
        html += '<p style="font-size:12px;margin-bottom:12px">或点击下方预设选项快速生成：</p>';
        html += '<div style="display:flex;flex-wrap:wrap;gap:6px;justify-content:center">';
        var presets = ['快速排序', '斐波那契数列', '二分查找', '冒泡排序', '阶乘计算', '判断质数', '反转字符串', '统计词频', '链表实现', '栈实现', '队列实现', 'HTTP GET请求'];
        presets.forEach(function(p) {
          html += '<button class="bce-btn bce-btn-sm bce-ai-gen-preset" style="font-size:11px;padding:4px 10px" data-desc="' + p + '">' + p + '</button>';
        });
        html += '</div>';
        html += '</div>';
        return html;
      ...
      html += '<p style="font-size:13px;margin-bottom:10px">根据描述「<strong>' + esc(desc) + '</strong>」生成的 ' + esc(CL.Languages.get(lang).name) + ' 代码：</p>';

      var generated = generateCodeFromDesc(desc, lang);
      html += '<pre class="bce-ai-code"><code>' + esc(generated) + '</code></pre>';
      html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:8px" data-code="' + encodeURIComponent(generated) + '">应用生成的代码</button>';

      html += '<div style="margin-top:14px;padding:10px 12px;background:var(--bg-soft);border-radius:var(--radius-sm);font-size:12px">';
      html += '<strong>支持的生成类型：</strong>排序算法、搜索算法、数学计算（斐波那契/阶乘/质数）、字符串处理、文件操作、数据结构（链表/栈/队列）、HTTP请求等。';
      html += '</div>';
      return html;
    }

    function generateCodeFromDesc(desc, lang) {
      var d = desc.toLowerCase();
      var L = CL.Languages.get(lang);

      // 快速排序
      if (/快速排序|quicksort|quick sort/.test(d)) {
        var qs = {
          ...
          go: 'package main\n\nimport "fmt"\n\nfunc quickSort(arr []int) []int {\n    if len(arr) <= 1 { return arr }\n    pivot := arr[len(arr)/2]\n    var left, middle, right []int\n    for _, v := range arr {\n        switch { case v < pivot: left = append(left, v); case v == pivot: middle = append(middle, v); default: right = append(right, v) }\n    }\n    return append(append(quickSort(left), middle...), quickSort(right)...)\n}\n\nfunc main() { fmt.Println(quickSort([]int{3, 6, 8, 10, 1, 2, 1})) }',
          rust: 'fn quick_sort(arr: &mut [i32]) {\n    if arr.len() <= 1 { return; }\n    let pivot = arr.len() / 2;\n    arr.swap(pivot, arr.len() - 1);\n    let mut i = 0;\n    for j in 0..arr.len() - 1 {\n        if arr[j] < arr[arr.len() - 1] { arr.swap(i, j); i += 1; }\n    }\n    arr.swap(i, arr.len() - 1);\n    quick_sort(&mut arr[..i]);\n    quick_sort(&mut arr[i + 1..]);\n}\n\nfn main() { let mut arr = [3, 6, 8, 10, 1, 2, 1]; quick_sort(&mut arr); println!("{:?}", arr); }'
        };
        return qs[lang] || qs.javascript;
      }

      // 斐波那契数列
      if (/斐波那契|fibonacci|fib/.test(d)) {
        var fib = {
          ...
          go: 'package main\n\nimport "fmt"\n\nfunc fibonacci(n int) int {\n    if n <= 1 { return n }\n    a, b := 0, 1\n    for i := 2; i <= n; i++ { a, b = b, a+b }\n    return b\n}\n\nfunc main() { for i := 0; i < 10; i++ { fmt.Print(fibonacci(i), " ") } }',
          rust: 'fn fibonacci(n: u32) -> u64 {\n    if n <= 1 { return n as u64; }\n    let (mut a, mut b) = (0u64, 1u64);\n    for _ in 2..=n { let temp = a + b; a = b; b = temp; }\n    b\n}\n\nfn main() { for i in 0..10 { print!("{} ", fibonacci(i)); } }'
        };
        return fib[lang] || fib.javascript;
      }

      // 二分查找
      if (/二分查找|binary search|binarysearch/.test(d)) {
        var bs = {
          python: 'def binary_search(arr, target):\n    left, right = 0, len(arr) - 1\n    while left <= right:\n        mid = (left + right) // 2\n        if arr[mid] == target:\n            return mid\n        elif arr[mid] < target:\n            left = mid + 1\n        else:\n            right = mid - 1\n    return -1\n\nprint(binary_search([1, 3, 5, 7, 9, 11], 7))',
          javascript: 'function binarySearch(arr, target) {\n  let left = 0, right = arr.length - 1;\n  while (left <= right) {\n    const mid = Math.floor((left + right) / 2);\n    if (arr[mid] === target) return mid;\n    if (arr[mid] < target) left = mid + 1;\n    else right = mid - 1;\n  }\n  return -1;\n}\nconsole.log(binarySearch([1, 3, 5, 7, 9, 11], 7));',
          java: 'public class BinarySearch {\n    public static int binarySearch(int[] arr, int target) {\n        int left = 0, right = arr.length - 1;\n        while (left <= right) {\n            int mid = left + (right - left) / 2;\n            if (arr[mid] == target) return mid;\n            if (arr[mid] < target) left = mid + 1;\n            else right = mid - 1;\n        }\n        return -1;\n    }\n    public static void main(String[] args) { System.out.println(binarySearch(new int[]{1,3,5,7,9,11}, 7)); }\n}',
          go: 'package main\n\nimport "fmt"\n\nfunc binarySearch(arr []int, target int) int {\n    left, right := 0, len(arr)-1\n    for left <= right {\n        mid := left + (right-left)/2\n        if arr[mid] == target { return mid }\n        if arr[mid] < target { left = mid + 1 } else { right = mid - 1 }\n    }\n    return -1\n}\n\nfunc main() { fmt.Println(binarySearch([]int{1,3,5,7,9,11}, 7)) }',
          rust: 'fn binary_search(arr: &[i32], target: i32) -> isize {\n    let (mut left, mut right) = (0isize, arr.len() as isize - 1);\n    while left <= right {\n        let mid = left + (right - left) / 2;\n        match arr[mid as usize].cmp(&target) { std::cmp::Ordering::Equal => return mid, std::cmp::Ordering::Less => left = mid + 1, std::cmp::Ordering::Greater => right = mid - 1 }\n    }\n    -1\n}\n\nfn main() { println!("{}", binary_search(&[1,3,5,7,9,11], 7)); }'
        };
        return bs[lang] || bs.javascript;
      }

      // 阶乘
      if (/阶乘|factorial/.test(d)) {
        var fact = {
          python: 'def factorial(n):\n    if n <= 1:\n        return 1\n    return n * factorial(n - 1)\n\nprint(factorial(5))',
          javascript: 'function factorial(n) { return n <= 1 ? 1 : n * factorial(n - 1); }\nconsole.log(factorial(5));',
          java: 'public class Factorial { public static long factorial(int n) { return n <= 1 ? 1 : n * factorial(n - 1); } public static void main(String[] args) { System.out.println(factorial(5)); } }',
          go: 'package main\nimport "fmt"\nfunc factorial(n int) int { if n <= 1 { return 1 }; return n * factorial(n-1) }\nfunc main() { fmt.Println(factorial(5)) }',
          rust: 'fn factorial(n: u64) -> u64 { if n <= 1 { 1 } else { n * factorial(n - 1) } }\nfn main() { println!("{}", factorial(5)); }'
        };
        return fact[lang] || fact.javascript;
      }

      // 回文判断
      if (/回文|palindrome/.test(d)) {
        var pal = {
          python: 'def is_palindrome(s):\n    s = s.lower().replace(" ", "")\n    return s == s[::-1]\n\nprint(is_palindrome("A man a plan a canal Panama"))',
          javascript: 'function isPalindrome(s) { s = s.toLowerCase().replace(/\\s/g, ""); return s === s.split("").reverse().join(""); }\nconsole.log(isPalindrome("A man a plan a canal Panama"));',
          java: 'public class Palindrome { public static boolean isPalindrome(String s) { s = s.toLowerCase().replaceAll("\\\\s", ""); return s.equals(new StringBuilder(s).reverse().toString()); } public static void main(String[] args) { System.out.println(isPalindrome("A man a plan a canal Panama")); } }',
          go: 'package main\nimport ("fmt"; "strings")\nfunc isPalindrome(s string) bool { s = strings.ToLower(strings.ReplaceAll(s, " ", "")); r := []rune(s); for i, j := 0, len(r)-1; i < j; i, j = i+1, j-1 { if r[i] != r[j] { return false } }; return true }\nfunc main() { fmt.Println(isPalindrome("A man a plan a canal Panama")) }',
          rust: 'fn is_palindrome(s: &str) -> bool { let s: String = s.to_lowercase().chars().filter(|c| !c.is_whitespace()).collect(); s == s.chars().rev().collect::<String>() }\nfn main() { println!("{}", is_palindrome("A man a plan a canal Panama")); }'
        };
        return pal[lang] || pal.javascript;
      }

      // 默认：生成一个通用的函数模板
      var defaultCode = {
        python: 'def ' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '():\n    """' + desc + '"""\n    pass\n\n' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '()',
        javascript: 'function ' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '() {\n  // ' + desc + '\n}\n\n' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '();',
        typescript: 'function ' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '(): void {\n  // ' + desc + '\n}\n\n' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '();',
        java: 'public class Main {\n    public static void ' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '() {\n        // ' + desc + '\n    }\n    public static void main(String[] args) {\n        ' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '();\n    }\n}',
        c: '#include <stdio.h>\n\nvoid ' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '() {\n    // ' + desc + '\n}\n\nint main() {\n    ' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '();\n    return 0;\n}',
        cpp: '#include <iostream>\nusing namespace std;\n\nvoid ' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '() {\n    // ' + desc + '\n}\n\nint main() {\n    ' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '();\n    return 0;\n}',
        go: 'package main\n\nimport "fmt"\n\nfunc ' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '() {\n    // ' + desc + '\n}\n\nfunc main() {\n    ' + (desc.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20) || 'myFunction') + '()\n}',
        rust: 'fn ' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '() {\n    // ' + desc + '\n}\n\nfn main() {\n    ' + (desc.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) || 'my_function') + '();\n}'
      };
      return defaultCode[lang] || defaultCode.javascript;
    }

    /* ---------- AI 代码审查 ---------- */
    function aiReview(code, lang, focus) {
      var stats = analyzeCode(code, lang);
      var syntax = checkSyntax(code, lang);
      var html = '<h4>代码审查报告</h4>';

      // 总体评分
      var overallScore = stats.qualityScore;
      var scoreColor = overallScore >= 80 ? '#10b981' : overallScore >= 60 ? '#f59e0b' : overallScore >= 40 ? '#f97316' : '#ef4444';
      html += '<div style="display:flex;align-items:center;gap:16px;margin:12px 0;padding:14px;background:var(--bg-soft);border-radius:var(--radius-sm)">';
      html += '<div style="width:64px;height:64px;border-radius:50%;background:' + scoreColor + ';display:flex;align-items:center;justify-content:center;color:#fff;font-size:24px;font-weight:700">' + overallScore + '</div>';
      html += '<div>';
      html += '<div style="font-size:16px;font-weight:600;margin-bottom:4px">代码质量：' + stats.qualityLevel + '</div>';
      html += '<div style="font-size:12px;color:var(--fg-mute)">圈复杂度 ' + stats.cyclomaticComplexity + ' · 嵌套深度 ' + stats.maxNesting + ' 层 · ' + stats.functions + ' 个函数 · ' + stats.classes + ' 个类</div>';
      html += '</div></div>';

      // 语法问题
      if (syntax.errors.length > 0 || syntax.warnings.length > 0) {
        html += '<div style="margin:12px 0">';
        html += '<p style="font-weight:600;margin-bottom:6px">🔴 语法问题（' + syntax.errors.length + ' 错误，' + syntax.warnings.length + ' 警告）</p>';
        syntax.errors.slice(0, 5).forEach(function(e) {
          html += '<div style="padding:6px 10px;margin:4px 0;background:#fef2f2;border-left:3px solid #ef4444;border-radius:0 4px 4px 0;font-size:12px">';
          html += '<span style="color:#ef4444;font-weight:600">错误</span> 第' + e.line + '行：' + esc(e.msg);
          html += '</div>';
        });
        syntax.warnings.slice(0, 3).forEach(function(w) {
          html += '<div style="padding:6px 10px;margin:4px 0;background:#fffbeb;border-left:3px solid #f59e0b;border-radius:0 4px 4px 0;font-size:12px">';
          html += '<span style="color:#f59e0b;font-weight:600">警告</span> 第' + w.line + '行：' + esc(w.msg);
          html += '</div>';
        });
        html += '</div>';
      }
      ...
      // 安全性审查
      html += '<div style="margin:12px 0">';
      html += '<p style="font-weight:600;margin-bottom:6px">🔒 安全性审查</p>';
      if (stats.securityIssues.length === 0) {
        html += '<div style="padding:8px 12px;background:#f0fdf4;border-left:3px solid #22c55e;border-radius:0 4px 4px 0;font-size:13px;color:#166534">未发现明显的安全问题</div>';
      } else {
        stats.securityIssues.forEach(function(issue) {
          html += '<div style="padding:6px 10px;margin:4px 0;background:#fef2f2;border-left:3px solid #ef4444;border-radius:0 4px 4px 0;font-size:12px">' + esc(issue) + '</div>';
        });
      }
      html += '</div>';
      ...
      // 性能审查
      html += '<div style="margin:12px 0">';
      html += '<p style="font-weight:600;margin-bottom:6px">⚡ 性能审查</p>';
      if (stats.perfIssues.length === 0) {
        html += '<div style="padding:8px 12px;background:#f0fdf4;border-left:3px solid #22c55e;border-radius:0 4px 4px 0;font-size:13px;color:#166534">未发现明显的性能问题</div>';
      } else {
        stats.perfIssues.forEach(function(issue) {
          html += '<div style="padding:6px 10px;margin:4px 0;background:#fffbeb;border-left:3px solid #f59e0b;border-radius:0 4px 4px 0;font-size:12px">' + esc(issue) + '</div>';
        });
      }
      html += '</div>';
      ...
      // 可维护性审查
      html += '<div style="margin:12px 0">';
      html += '<p style="font-weight:600;margin-bottom:6px">🔧 可维护性审查</p>';
      var maintainIssues = [];
      if (stats.commentLines === 0 && stats.codeLines > 10) maintainIssues.push('代码无注释，建议为复杂逻辑添加注释');
      if (stats.maxNesting > 3) maintainIssues.push('嵌套深度 ' + stats.maxNesting + ' 层，建议用提前返回减少嵌套');
      if (stats.cyclomaticComplexity > 10) maintainIssues.push('圈复杂度 ' + stats.cyclomaticComplexity + '，建议拆分函数降低复杂度');
      if (stats.avgLineLen > 100) maintainIssues.push('平均行长度 ' + stats.avgLineLen + ' 字符，建议拆分行提升可读性');
      if (stats.codeSmells.length > 0) maintainIssues = maintainIssues.concat(stats.codeSmells);
      if (maintainIssues.length === 0) {
        html += '<div style="padding:8px 12px;background:#f0fdf4;border-left:3px solid #22c55e;border-radius:0 4px 4px 0;font-size:13px;color:#166534">代码可维护性良好</div>';
      } else {
        maintainIssues.forEach(function(issue) {
          html += '<div style="padding:6px 10px;margin:4px 0;background:#eff6ff;border-left:3px solid #3b82f6;border-radius:0 4px 4px 0;font-size:12px">' + esc(issue) + '</div>';
        });
      }
      html += '</div>';
      ...
      // 最佳实践检查清单
      html += '<div style="margin:12px 0">';
      html += '<p style="font-weight:600;margin-bottom:6px">✅ 最佳实践检查</p>';
      var checks = [
        { pass: syntax.errors.length === 0, text: '无语法错误' },
        { pass: stats.securityIssues.length === 0, text: '无明显安全漏洞' },
        { pass: stats.maxNesting <= 3, text: '嵌套深度不超过3层' },
        { pass: stats.cyclomaticComplexity <= 10, text: '圈复杂度不超过10' },
        { pass: stats.commentLines > 0 || stats.codeLines <= 10, text: '有适当的注释' },
        { pass: !/eval\s*\(/.test(code), text: '未使用 eval()' },
        { pass: !/console\.log\s*\(/.test(code) || stats.codeLines <= 30, text: '无遗留的调试语句' }
      ];
      html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">';
      checks.forEach(function(c) {
        html += '<div style="display:flex;align-items:center;gap:6px;font-size:12px">';
        html += c.pass ? '<span style="color:#22c55e">✓</span>' : '<span style="color:#ef4444">✗</span>';
        html += '<span style="' + (c.pass ? '' : 'color:var(--fg-mute);text-decoration:line-through') + '">' + c.text + '</span>';
        html += '</div>';
      });
      html += '</div></div>';
      ...
      // 应用按钮区域
      html += '<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">';
      html += '<p style="font-size:13px;margin-bottom:8px">根据审查建议，可一键应用优化后的代码：</p>';
      ...
      // 应用格式化版本
      var formattedReview = code
        .replace(/[ \t]+$/gm, '')
        .replace(/\n{3,}/g, '\n\n')
        .replace(/^\n+|\n+$/g, '');
      html += '<button class="bce-btn bce-btn-sm bce-ai-apply" data-code="' + encodeURIComponent(formattedReview) + '">应用格式化版本</button>';
      html += '<button class="bce-btn bce-btn-sm bce-ai-apply" data-code="' + encodeURIComponent(code) + '">重新应用当前代码</button>';
      html += '</div></div>';

      html += '<p style="margin-top:12px;color:var(--fg-mute);font-size:12px">以上为静态分析结果，建议结合实际运行测试和代码评审。</p>';
      return html;
    }

    /* ---------- AI 单元测试生成 ---------- */
    function aiTest(code, lang, framework) {
      var html = '<h4>单元测试生成</h4>';
      var functions = extractFunctions(code, lang);

      if (functions.length === 0) {
        html += '<div style="padding:16px;text-align:center;color:var(--fg-mute)">';
        html += '<p style="font-size:14px;margin-bottom:8px">未检测到可测试的函数</p>';
        html += '<p style="font-size:12px;margin-bottom:12px">请确保代码中包含函数定义，如 Python 的 def、JavaScript 的 function 等</p>';
        html += '<p style="font-size:12px;margin-bottom:8px">或点击下方按钮应用基础测试模板：</p>';
        // 基础测试模板
        var testTemplates = {
          python: 'import unittest\n\nclass TestMain(unittest.TestCase):\n    def test_example(self):\n        # 示例测试用例\n        self.assertEqual(1 + 1, 2)\n\nif __name__ == \'__main__\':\n    unittest.main()',
          javascript: '// 基础测试模板（使用 Jest）\ndescribe(\'基础测试\', () => {\n  test(\'示例测试\', () => {\n    expect(1 + 1).toBe(2);\n  });\n});',
          typescript: '// 基础测试模板（使用 Jest）\ndescribe(\'基础测试\', () => {\n  test(\'示例测试\', () => {\n    expect(1 + 1).toBe(2);\n  });\n});',
        ...
        var templateCode = testTemplates[lang] || testTemplates.javascript;
        html += '<pre class="bce-ai-code" style="text-align:left"><code>' + esc(templateCode) + '</code></pre>';
        html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:8px" data-code="' + encodeURIComponent(templateCode) + '">应用基础测试模板</button>';
        html += '</div>';
        return html;
      }

      html += '<p style="font-size:13px;margin-bottom:10px">检测到 <strong>' + functions.length + '</strong> 个函数，为其生成单元测试：</p>';
      functions.forEach(function(fn) {
        html += '<div style="padding:6px 10px;margin:4px 0;background:var(--bg-soft);border-radius:var(--radius-sm);font-size:12px">';
        html += '<span style="font-weight:600">' + esc(fn.name) + '</span>(' + esc(fn.params) + ')';
        html += '</div>';
      });

      var testCode = generateTestCode(functions, lang, code);
      html += '<p style="margin:12px 0 6px;font-weight:600">生成的测试代码</p>';
      html += '<pre class="bce-ai-code"><code>' + esc(testCode) + '</code></pre>';
      html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:8px" data-code="' + encodeURIComponent(testCode) + '">应用测试代码</button>';

      html += '<div style="margin-top:14px;padding:10px 12px;background:var(--bg-soft);border-radius:var(--radius-sm);font-size:12px">';
      html += '<strong>运行测试：</strong>';
      var runCmds = {
        python: 'pytest test_main.py -v',
        javascript: 'npx jest main.test.js',
        typescript: 'npx jest main.test.ts',
        java: 'mvn test 或 java -cp .:junit.jar org.junit.runner.JUnitCore MainTest',
        c: 'gcc -o test test_main.c && ./test',
        cpp: 'g++ -o test test_main.cpp && ./test',
        go: 'go test -v',
        rust: 'cargo test'
      };
      html += '<code style="background:var(--code-bg);padding:2px 6px;border-radius:3px">' + (runCmds[lang] || '运行对应测试框架') + '</code>';
      html += '</div>';
      return html;
    }
    ...
    function extractFunctions(code, lang) {
      ...
      var pattern = patterns[lang] || patterns.javascript;
      var m;
      while ((m = pattern.exec(code)) !== null) {
        if (m[1] !== 'main' && m[1] !== 'Main') {
          funcs.push({ name: m[1], params: m[2] || '' });
        }
      }
      return funcs;
    }

    function generateTestCode(functions, lang, originalCode) {
      var fn = functions[0] || { name: 'myFunction', params: '' };
      var tests = {
        python: 'import pytest\nfrom main import ' + fn.name + '\n\nclass Test' + fn.name.charAt(0).toUpperCase() + fn.name.slice(1) + ':\n    def test_' + fn.name + '_basic(self):\n        """测试基本功能"""\n        result = ' + fn.name + '()\n        assert result is not None\n\n    def test_' + fn.name + '_edge_case(self):\n        """测试边界情况"""\n        pass\n\nif __name__ == "__main__":\n    pytest.main([__file__, "-v"])',
        javascript: 'const { ' + fn.name + ' } = require("./main");\n\ndescribe("' + fn.name + '", () => {\n  test("基本功能测试", () => {\n    const result = ' + fn.name + '();\n    expect(result).toBeDefined();\n  });\n\n  test("边界情况测试", () => {\n    // TODO: 添加边界情况测试\n  });\n});',
        typescript: 'import { ' + fn.name + ' } from "./main";\n\ndescribe("' + fn.name + '", () => {\n  test("基本功能测试", () => {\n    const result = ' + fn.name + '();\n    expect(result).toBeDefined();\n  });\n\n  test("边界情况测试", () => {\n    // TODO: 添加边界情况测试\n  });\n});',
        java: 'import org.junit.Test;\nimport static org.junit.Assert.*;\n\npublic class MainTest {\n    @Test\n    public void test' + fn.name.charAt(0).toUpperCase() + fn.name.slice(1) + 'Basic() {\n        // 测试基本功能\n        assertNotNull(Main.' + fn.name + '());\n    }\n\n    @Test\n    public void test' + fn.name.charAt(0).toUpperCase() + fn.name.slice(1) + 'EdgeCase() {\n        // TODO: 添加边界情况测试\n    }\n}',
        c: '#include <stdio.h>\n#include <assert.h>\n\nextern int ' + fn.name + '();\n\nvoid test_' + fn.name + '_basic() {\n    printf("测试 ' + fn.name + ' 基本功能... ");\n    assert(' + fn.name + '() != 0);\n    printf("通过\\n");\n}\n\nint main() {\n    test_' + fn.name + '_basic();\n    printf("所有测试通过！\\n");\n    return 0;\n}',
        cpp: '#include <iostream>\n#include <cassert>\n\nextern int ' + fn.name + '();\n\nvoid test_' + fn.name + '_basic() {\n    std::cout << "测试 ' + fn.name + ' 基本功能... ";\n    assert(' + fn.name + '() != 0);\n    std::cout << "通过" << std::endl;\n}\n\nint main() {\n    test_' + fn.name + '_basic();\n    std::cout << "所有测试通过！" << std::endl;\n    return 0;\n}',
        go: 'package main\n\nimport "testing"\n\nfunc Test' + fn.name.charAt(0).toUpperCase() + fn.name.slice(1) + '(t *testing.T) {\n    // 测试基本功能\n    result := ' + fn.name + '()\n    if result == nil {\n        t.Errorf("' + fn.name + '() 返回 nil")\n    }\n}\n\nfunc Test' + fn.name.charAt(0).toUpperCase() + fn.name.slice(1) + 'EdgeCase(t *testing.T) {\n    // TODO: 添加边界情况测试\n}',
        rust: '#[cfg(test)]\nmod tests {\n    use super::*;\n\n    #[test]\n    fn test_' + fn.name + '_basic() {\n        // 测试基本功能\n        let result = ' + fn.name + '();\n        assert!(result.is_some() || true);\n    }\n\n    #[test]\n    fn test_' + fn.name + '_edge_case() {\n        // TODO: 添加边界情况测试\n    }\n}'
      };
      return tests[lang] || tests.javascript;
    }

    /* ---------- AI 代码重构 ---------- */
    function aiRefactor(code, lang, target) {
      var stats = analyzeCode(code, lang);
      var html = '<h4>代码重构</h4>';
      var refactored = code;
      var changes = [];
      ...
      var duplicatedLines = Object.keys(lineCounts).filter(function(k) { return lineCounts[k] > 1; });
      if (duplicatedLines.length > 0) {
        changes.push({ type: 'extract', text: '检测到 ' + duplicatedLines.length + ' 行重复代码，建议提取为函数' });
      }

      // 2. 重命名短变量名
      var shortVars = code.match(/\b(?:let|var|const|int|float|double|char|bool|auto)\s+([a-z]{1,2})\s*=/g);
      if (shortVars && shortVars.length > 0) {
        changes.push({ type: 'rename', text: '检测到 ' + shortVars.length + ' 个短变量名（如 a, b, x），建议使用有意义的名称' });
      }

      // 3. 简化嵌套（提前返回）
      if (stats.maxNesting > 3) {
        changes.push({ type: 'flatten', text: '嵌套深度 ' + stats.maxNesting + ' 层，建议用提前返回（guard clause）扁平化' });
        // 简单的提前返回重构（仅对JS/TS/Python）
        if (lang === 'javascript' || lang === 'typescript' || lang === 'python') {
          refactored = refactored.replace(/if\s*\(([^)]+)\)\s*\{\s*([\s\S]*?)\s*\}\s*else\s*\{/g, function(m, cond, body) {
            return 'if (!(' + cond + ')) {\n  return;\n}\n' + body + '\n{';
          });
          hasRefactor = true;
        }
      ...
      // 4. 移除死代码（注释掉的代码）
      var deadCode = code.match(/^\s*\/\/\s*(if|for|while|return|var|let|const|function|def|class)\s+/gm);
      if (deadCode && deadCode.length > 0) {
        changes.push({ type: 'remove', text: '检测到 ' + deadCode.length + ' 处注释掉的代码，建议移除' });
      }

      // 5. 统一引号风格
      if (lang === 'javascript' || lang === 'typescript') {
        var singleQuotes = (code.match(/'/g) || []).length;
        var doubleQuotes = (code.match(/"/g) || []).length;
        if (singleQuotes > 0 && doubleQuotes > 0 && Math.abs(singleQuotes - doubleQuotes) > 3) {
          changes.push({ type: 'consistent', text: '引号风格不统一，建议统一使用单引号或双引号' });
        }
      }

      // 6. 自动优化（复用aiOptimize的规则）
      if (lang === 'javascript' || lang === 'typescript') {
        if (/\bvar\s+\w+/.test(refactored)) {
          refactored = refactored.replace(/\bvar\s+/g, 'let ');
          changes.push({ type: 'modernize', text: 'var → let（现代JS语法）' });
          hasRefactor = true;
        }
        if (/==[^=]/.test(refactored)) {
          refactored = refactored.replace(/==([^=])/g, '===$1');
          changes.push({ type: 'modernize', text: '== → ===（严格相等）' });
          hasRefactor = true;
        }
      }
      ...
      // 输出重构建议
      if (changes.length > 0) {
        html += '<p style="font-size:13px;margin-bottom:8px">发现 <strong>' + changes.length + '</strong> 项重构建议：</p>';
        html += '<ul style="margin:8px 0;padding-left:18px">';
        changes.forEach(function(c) {
          var icon = { extract: '📦', rename: '🏷️', flatten: '📏', remove: '🗑️', consistent: '🎯', modernize: '✨' }[c.type] || '💡';
          html += '<li style="margin:4px 0;font-size:13px">' + icon + ' ' + c.text + '</li>';
        });
        html += '</ul>';
      } else {
        html += '<div style="padding:16px;text-align:center;color:var(--fg-mute)">';
        html += '<p style="font-size:14px">代码结构良好，未发现明显的重构需求</p>';
        html += '</div>';
      }

      // 重构后的代码
      if (hasRefactor) {
        html += '<p style="margin:12px 0 6px;font-weight:600">自动重构后的代码</p>';
        html += '<p style="font-size:12px;color:var(--fg-mute);margin-bottom:6px">已自动应用可安全修改的重构，其他建议需手动确认：</p>';
        html += '<pre class="bce-ai-code"><code>' + esc(refactored) + '</code></pre>';
        html += '<button class="bce-btn bce-btn-sm bce-ai-apply" style="margin-top:8px" data-code="' + encodeURIComponent(refactored) + '">应用重构后的代码</button>';
      } else {
        // 即使没有可自动重构的问题，也提供应用按钮
        html += '<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">';
        html += '<p style="font-size:13px;margin-bottom:8px">代码结构良好，未发现可自动重构的问题，可点击下方按钮重新应用当前代码：</p>';
        html += '<button class="bce-btn bce-btn-sm bce-ai-apply" data-code="' + encodeURIComponent(code) + '">重新应用当前代码</button>';
        html += '</div>';
      }

      html += '<p style="margin-top:12px;color:var(--fg-mute);font-size:12px">重构前请确保有版本控制，重构后运行测试验证功能不变。</p>';
      return html;
    }
    ...
    function toggleFullscreen() {
      ...
      else { host.requestFullscreen().catch(function () {}); }
    }
    function onFsChange() {
      ...
      if (editor && editor.resize) { try { editor.resize(); } catch (e) {} }
    }
    palInput.addEventListener('input', function () { palSel = 0; renderPalette(palInput.value); });
    palInput.addEventListener('keydown', function (e) {
      var items = palList.querySelectorAll('.bce-pal-item');
      if (!items.length) return;
      if (e.key === 'ArrowDown') { e.preventDefault(); palSel = Math.min(palSel + 1, items.length - 1); renderPalette(palInput.value); }
      else if (e.key === 'ArrowUp') { e.preventDefault(); palSel = Math.max(palSel - 1, 0); renderPalette(palInput.value); }
      else if (e.key === 'Enter') {
        e.preventDefault();
        var list = COMMANDS.filter(function (c) { return c.t.toLowerCase().indexOf((palInput.value || '').toLowerCase()) >= 0; });
        var cmd = list[palSel];
    ...
    function onKey(e) {
      ...
      // 焦点在 Ace 编辑器内时，Ctrl+Enter / Ctrl+Shift+P 已由 Ace 命令处理，避免重复触发
      var inAce = !usingFallback && editorHost.contains(e.target);
      ...
      else if ((e.ctrlKey || e.metaKey) && e.key === 's') { e.preventDefault(); saveActive(); }
      else if ((e.ctrlKey || e.metaKey) && e.key === 'n') { e.preventDefault(); newFile(); }
    }

    /* ---------- 启动 ---------- */
    activeId = files[0].id;
    langSel.value = activeFile().lang;
    nameInput.value = activeFile().name;
    renderTabs(); renderFiles(); renderConsole();
    buildEditor();

    var api = {
      destroy: function () {
        destroyed = true;                          // 先置位：任何在途的 mode/theme 回调立即被守卫丢弃
        host.removeEventListener('keydown', onKey);
        document.removeEventListener('fullscreenchange', onFsChange);
        if (editor && editor._ro) { try { editor._ro.disconnect(); } catch (e) {} }
        if (editor && editor._mo) { try { editor._mo.disconnect(); } catch (e) {} }
        if (editor && editor.destroy) editor.destroy();
        uninstallLoadModuleGuard(global.ace);       // 还原全局 loadModule，避免影响后续实例
        if (host.parentNode) host.parentNode.removeChild(host);
      },
      setLang: function (lang) { langSel.value = lang; changeLang(); },
      newFile: newFile,
      run: runActive,
      // 从外部（如代码卡片）带一段代码进来继续编辑：新建一个对应语言文件并填入代码
      openCode: function (lang, code) {
        var L = CL.Languages.get(lang);
        if (!L) { if (CL.toast) CL.toast('该语言暂不支持在编辑器中打开', 'warn'); return; }
        langSel.value = L.id; newFile();
        setCode(code || '');
        // 联动动画：代码飞入效果
        host.classList.add('bce-editor-loaded-from-card');
        showLoadedFromCardBanner();
      ...
      setCode: function (code) {
        var f = activeFile(); if (!f) return;
        if (usingFallback) editor.setValue(code);
        else if (sessions[f.id]) sessions[f.id].setValue(code);
        else if (editor.setValue) editor.setValue(code);
        syncActiveFromEditor();
      },
      getActive: function () {
        var f = activeFile();
        return f ? { id: f.id, name: f.name, lang: f.lang, value: f.value, dirty: !!f.dirty } : null;
      }
    };
    CL.BytecodeEditor._active = api;

    return api;
  }

  CL.BytecodeEditor = { mount: Mount, _active: null, pending: null };

  /* 从任意代码卡片「打开编辑器继续编辑」：
     - 若当前已在 Playground，直接把代码作为新文件载入当前实例（不会因 hash 相同而不刷新）
     - 否则暂存待编辑代码并跳转到 #/playground，由 Mount 在 loadFiles 时消费 */
  CL.BytecodeEditor.openWith = function (lang, code) {
    if (location.hash === '#/playground' && CL.BytecodeEditor._active) {
      CL.BytecodeEditor._active.openCode(lang, code);
      return;
    }
    CL.BytecodeEditor.pending = { lang: lang, code: code || '' };
